...
*Consequence (added M15, 2026-09-09; PR #162)* — **The single-channel core is amended for one
opt-in mode, and the requirement's own Rationale is what the amendment is measured against.** The
Rationale reads "forcing stereo through the core would double the cost for no benefit". Both halves
are now measured facts rather than predictions, and they point in opposite directions for two
different cases:

- *No benefit* is false for one case the Rationale does not reach: a `ChannelConfig::Stereo` input
  whose two channels carry two genuinely different signals — the standing example is a DAW parent
  bus summing two panned, double-tracked guitar takes. There, the mono core does not widen one
  source; it destroys one of two performances before the amp sees it (Trim's −6 dB sum, or the
  gate-then-single-curve path upstream of it). The benefit is the second take.
- *Double the cost* is true, and slightly worse than "double". **Certified on
  `02-architecture.md` §2's reference machine, quiet, with `benches/six_stage_chain.rs`** (5
  repetitions per mode, **every one of the ten passing D-2.4's own validity check**, so both of
  that binary's statistics are quotable rather than only the contamination-immune one):

  | statistic | Linked (shipped) | Independent | ratio |
  |---|---|---|---|
  | per-residue-minimum estimator | 14.40% of one core | **28.35-29.07%** | 1.97-2.02x |
  | raw `p99.9` (D-2.2's own gate) | 16.76% | **31.39-31.82%** | 1.87-1.90x |
  | `p50` | 7.40% | 15.07% | 2.04x |

  Both statistics are **over NFR-PERF-010's 25% budget**, so this is not a reading that depends on
  which estimator one prefers. An earlier set, taken while this session's own agent tooling ran and
  therefore disqualified by D-2.4 condition 2, is kept on the record in `02-architecture.md`
  D-9.14: it read 15.03% / 14.72% against 30.08% / 30.86% with every repetition discarded, so the
  quiet re-run moved the estimator by under two points and the conclusion not at all — which is the
  argument for that estimator made by the instrument rather than about it.

**So the amendment is deliberately narrow.** The core remains single-channel in every shipped
configuration and in every default. `global.independent_channels` (`namir-params`, "Linked" /
"Independent", default "Linked") is the only way to leave that behaviour, it is reachable only in
the `Stereo` configuration — `Mono` and `MonoToStereo` capture one channel, so the engine does not
even build the per-channel state for them and the setting is inert, including for a preset written
by the plugin and opened in the standalone — and turning it on is a user's decision to spend
headroom NFR-PERF-010 does not grant. What this note **does not** do: it does not amend
NFR-PERF-010, whose 25% budget continues to be stated and gated for the shipped configuration
only (`six_stage_chain.rs` asserts it in Linked mode and reports without asserting in
Independent mode), and it does not make the independent mode a Must. FR-CHAIN-060 and
FR-CHAIN-070 are unchanged: the amendment adds a mode *outside* that table's Stereo row rather
than choosing between the two inputs that row already permits. See `02-architecture.md` **D-9.14**
for the design, its per-channel state cost, and §22 **R-19** for the risk this leaves open.
...
shall have an accessible name, exposed to platform assistive technology on Windows and macOS (Linux/X11 is a compiled no-op, Decision D-15.4).
...
*Update (2026-09-09, issue #35)*: The open question this note records is settled by Decision D-15.4 — AccessKit adapters are wired for Windows and macOS, Linux/X11 is a compiled no-op. The manual script's execution, including the keyboard-operability half, remains owed; see the Consequence below.
...
| 0.10 | 2026-09-09 | FR-UI-030 accessibility scope narrowed (issue #35, PR #188). **FR-UI-030 is amended** to explicitly specify assistive technology exposure on Windows and macOS, with Linux/X11 recorded as a compiled no-op, matching Decision D-15.4. Appended M14 note marked settled with respect to adapter implementation. |
| 0.11 | 2026-09-09 | **FR-CHAIN-050 is amended for one opt-in mode** (PR #162), by an appended `*Consequence (added M15, 2026-09-09)*` note; the original text, Rationale and `*Verify:*` line stand. The core remains single-channel in every shipped configuration and every default. `global.independent_channels` ("Linked" / "Independent", default "Linked") lets a user run Gate, Nam and Ir per channel and skip Trim's downmix, for the case the Rationale's "no benefit" does not reach: a `Stereo` input carrying two genuinely different signals, e.g. a DAW parent bus summing two panned double-tracked takes, where the mono core destroys one performance rather than widening one source. The Rationale's *other* half is confirmed and quantified rather than contradicted — **certified** on §2's reference machine, quiet, with `six_stage_chain.rs` — 5 repetitions per mode, all ten passing D-2.4's validity check, so both gate statistics are quotable: the estimator reads 14.40% of one core Linked against **28.35-29.07%** with the mode on (two quiet runs), and raw `p99.9` 16.76% against **31.39-31.82%** — ~2x, and **over NFR-PERF-010's 25% budget on both statistics**. An earlier, contaminated set (every repetition discarded under D-2.4 condition 2) read 15.03% / 14.72% against 30.08% / 30.86% and is kept on the record at D-9.14: the estimator moved under two points between the two runs, the conclusion not at all. The mode is reachable only in the `Stereo` configuration, so it is inert in the standalone and a plugin-written preset carrying it is inert there too — FR-STATE-030 portability and FR-CFG-020 parity both preserved. **NFR-PERF-010 is not amended**: its budget stays stated and gated for the shipped configuration, which is what `six_stage_chain.rs` asserts in Linked mode and reports without asserting in Independent mode. FR-CHAIN-060 and FR-CHAIN-070 unchanged. New citation for FR-PARAM-050's "channel mode" category, which was vacuous until now. See `02-architecture.md` **D-9.14** and §22 **R-19**. |
