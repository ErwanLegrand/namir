...
embedding clause remains `**PARTIAL**` in `docs/05-test-plan.md`.
...
2. `05-test-plan.md`, mapped to one or more test identifiers;
...
*Consequence (added M7)* — `05-test-plan.md` never existed as a separate file: the implementation
...
`docs/05-test-plan.md` is now itself **generated** by `cargo run -p xtask -- traceability --write`
...
recognises two marker spellings — `// trace:` and `# trace:` — and `xtask/src/main.rs:244-255`
...
`docs/05-test-plan.md` records all fifteen as covered by `ci` or `workspace`. Enforcing the sentence
...
*Consequence (added 2026-09-21) — the plan was renumbered, and point 2 above is corrected in
place.* `docs/03-test-plan.md` and `docs/03-implementation-roadmap.md` both carried the "03" slot,
a collision the M7 note above records from the plan's side. This pass moves the **plan** to
`docs/05-test-plan.md`, so `docs/` reads 01 (this document), 02 (architecture), 03 (roadmap), 04
(state format), 05 (test plan). Point 2 is edited rather than left standing, because a mapping list
naming a file that does not exist is a defect and not a record; the notes above keep the former
spelling, being the record of what was true when they were written, and they name this same file.
**The plan moved rather than the roadmap**: the roadmap's number is cited as provenance by 39 files
(benches, fuzz targets, crate doc comments) against the plan's 11, and the plan is generated with a
body that never names its own file — so this changes no generated byte, and `cargo run -p xtask --
traceability` regenerates it identically at the new path. The rename and `xtask/src/main.rs`'s path
constant must land in one commit, or D-18.5's required plan-diff half goes red.
...
| 0.11 | 2026-09-21 | **`docs/03-test-plan.md` renumbered to `docs/05-test-plan.md`**, closing the two-`03` collision Section 10's M7 note records: the roadmap took the "03" slot at M1 and the plan was created at "03" anyway at M7. Section 10's point 2 is corrected in place and a `*Consequence (added 2026-09-21)*` note appended; every path reference in the tree is updated in the same commit. The plan moved rather than the roadmap because the roadmap's number is cited as provenance by 39 files against the plan's 11, and the plan's generated body never names its own file, so the rename changes no generated byte. |
