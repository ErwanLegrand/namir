...
- *Double the cost* is true, and slightly worse than "double". Measured on
  `02-architecture.md` §2's reference machine with `benches/six_stage_chain.rs`, 5 repetitions per
  mode per set, two interleaved sets, on the contamination-immune per-residue-minimum estimator
  D-2.4 promotes for exactly this comparison: **15.03% / 14.72% of one core in the shipped Linked
  mode against 30.08% / 30.86% with the mode on** — a factor of 2.00–2.10, and **over
  NFR-PERF-010's 25% budget**. The machine carried this session's own agent tooling, so D-2.4
  condition 2 disqualifies the figures as *certified*; the ratio is same-session and interleaved,
  which is the comparison D-2.1's own note says to trust on this machine.
