...
*Consequence (added M9b, 2026-08-12)* — **`namir-clap` now carries a `namir-app` dev-dependency,
which the "everything except app" cell above forbids as a normal edge and this note records
deliberately rather than leaving to be discovered.** FR-CFG-020 ("bit-identical output in both
configurations") is the one Must whose subject is both product shells at once, so its test has to
drive both, and a bit-identity claim only means anything inside one process and one build — the
chain reaches `f32::tanh`/`exp`/`sin`/`cos` through the platform's libm, so splitting the
comparison across two test binaries via a checked-in expected-output file would be asserting
cross-platform bit-exactness of libm, which this product does not claim and CI's Linux and macOS
runners would be entitled to fail on. Only one direction is expressible, since `namir-app`'s row
does not name `namir-clap` at all, so the test lives in
`crates/namir-clap/tests/fr_cfg_020_shell_parity.rs`. What the cell protects is untouched: it
exists so the shipped plugin cannot require the standalone application to be installed
(FR-CFG-030), and a dev-dependency is linked into no shipped artifact — the `cdylib` reaches
nothing new, `xtask layering` checks normal edges only and stays green on its own terms rather than
by exemption, and `xtask attribution`'s normal-edge walk adds no THIRD-PARTY-NOTICES.md row
(verified by running both, not assumed). The mirror-image argument `xtask/src/layering.rs`'s own
doc comment makes for FR-CFG-030's other half — that `namir-app`'s row never names `namir-clap` —
remains literally true, and is precisely why the parity test lives on this side of the boundary.
The cost is that `cargo test -p namir-clap` now builds `namir-app` and therefore `cpal`.
...
`crates/namir-platform/src/thread_priority.rs:46` and `crates/namir-clap/src/gui.rs:80` legal. Cargo
...
`crates/namir-worker/tests/rt_stress.rs:65-66` installs it inside a crate taking the workspace
...
*Consequence (added M15, 2026-09-07, issue #94)* — a **fourth** designated module lands:
`crates/namir-clap/src/host_wake.rs`. Issue #94's preset recall runs on a worker-pool thread and
must ask the host for an `on_main_thread` callback (`HostParams::rescan` is `[main-thread]`), but
the `HostSharedHandle<'a>` clack hands to `new_shared` is `'a`-bound while pool closures must be
`'static`. `host_wake.rs` stores the raw `clap_host` behind an erased `HostSharedHandle<'static>`
and carries the written safety argument D-5.3 requires. The workspace's `unsafe` surface is now
**four files across the same two crates**: `namir-platform/src/denormal.rs`,
`namir-platform/src/thread_priority.rs`, `namir-clap/src/gui.rs` and `namir-clap/src/host_wake.rs`.
The argument pins soundness on three facts: the `clap_host` pointer is guaranteed by the CLAP spec
to outlive the plugin instance until `plugin->destroy` returns; `NamirShared::drop` joins every
worker thread (through `pool.shutdown`) before `destroy` returns and `ThreadPool::spawn` drops any
newly dispatched jobs post-shutdown under lock, so no pool thread holding the erased handle can call
it after the pointer dies; and `request_callback` is one of `clap_host`'s explicitly thread-safe
...
*Consequence (added M9b, 2026-08-12)* — **The consequence above is about buffer *sizing*, and
FR-CLAP-070 turned out to need more than that.** M9b ran the requirement against a loaded chain for
the first time — every prior block-size test drove a resource-free default chain — and found
`namir-engine`'s `SlotResampler` violating the requirement while every buffer was correctly sized.
Its produce loop primed the output FIFO to *this call's* output length only and emitted
`unwrap_or(0.0)` when starved, so a small block spliced in a sample of silence that was never taken
back: the stage's delay became a function of block-size *history*, growing to 63 samples under
one-frame blocks and never shrinking, and the shift arrived **mid-stream** when a host changed
block size. "A smaller block simply uses a prefix" is true of the buffers and false of a stateful
stage's delay. The general form, worth stating because nothing in this decision implied it: **a
stage carrying its own FIFOs owes a block-size-independent delay invariant, which buffer sizing
does not supply and cannot check.** The fix establishes one — priming `engine_out_fifo` with a full
`engine_block` so `engine_in_fifo.len() + engine_out_fifo.len() == engine_block` holds for the life
of the slot, under which the produce loop cannot starve (`crates/namir-engine/src/stages/nam.rs`,
`SlotResampler::new` and `NamStage::reset`). D-6.2's own claim is not retracted: the design does
handle arbitrary block sizes, and what M9b reported was an implementation that had diverged from
it, which is why roadmap §15 item 13's branch policy sent the failure to the engine rather than to
the FRS.
...
*Consequence (added M9b, 2026-08-12)* — The measurement this decision asks for was built at M9b,
having existed nowhere until then, and **both** resamplers failed it as configured; the "not by
trusting the library's defaults" clause turned out to be the load-bearing half of the decision.
The instrument is `namir_fixtures::resample_response` (coherent-sampling multitone, shared so the
two sites are measured by one yardstick), driven from
`namir-engine`'s `resampler_frequency_response_meets_the_stopband_and_ripple_bar` and `namir-ir`'s
`resampler_frequency_response_meets_the_ir_quality_bar`. What it found, in both cases, is that
`rubato` places an antialiasing filter's cutoff as a *fraction of the lower Nyquist that depends
on the filter's length* (`rubato::calculate_cutoff`), so an under-length filter loses passband
rather than merely widening its transition: the NAM stage's `FftFixedInOut` pair, whose filter
length is its FFT size, measured **−15 dB at 20 kHz** for a 48 kHz model at a 192 kHz engine rate
and −5.6 dB for a 44.1 kHz model at 96 kHz. The fix is `MIN_RESAMPLE_FFT_FRAMES`: size the chunk
so the FFT is ≥ 256 frames *in the lower rate's domain* rather than a flat 256 engine frames. The
48 kHz engine cases, including 48 kHz against a 44.1 kHz model, are unchanged by it — where the
engine rate is the lower of the two the old and new rules coincide — so what it costs is latency at
engine rates *above* the model rate, where the internal block becomes a constant ~5.3 ms rather
than a constant 256 frames (1.3 ms at 192 kHz). That is the same latency a 48 kHz session already
carries, and it is reported through the same `latency_samples` path FR-CLAP-040 reads.

*Consequence (added M9b, 2026-08-12)* — The same measurement retires the "sinc" half of this
decision's own wording for the IR path. `namir-ir`'s `resample_mono` was `SincFixedIn`
(`sinc_len = 256`, `BlackmanHarris2`, `rubato::calculate_cutoff`'s own cutoff) and measured
**−17.7 dB** of stopband attenuation at 192 → 48 kHz and −35.0 dB at 96 → 44.1 kHz against the
100 dB bar FR-IR-030 imports from FR-NAM-060 — because a windowed sinc's transition band straddles
its cutoff and therefore extends past the lower Nyquist, where it aliases. Lengthening the sinc
fixes it (`sinc_len = 2048`, `f_cutoff = 0.96` measured −115 dB) at several times the cost; it now
uses `FftFixedInOut` with the same `MIN_RESAMPLE_FFT_FRAMES` rule as the NAM stage instead, which
truncates the spectrum *at* the lower Nyquist by construction and measures ≥ 129 dB with ~1e-5 dB
of ripple. So both of Namir's resamplers are now FFT resamplers; nothing uses `rubato`'s sinc
family, and a future site that wants one owes this measurement first.

*Consequence (added M9b, 2026-08-12)* — FR-NAM-060 is **not** fully closed by this, and the residue
is in the requirement rather than the code. "Passband ripple no greater than 0.1 dB up to 20 kHz or
the Nyquist frequency, whichever is lower" is unsatisfiable by *any* resampler once the lower of
the two rates falls below about 40 kHz, since the Nyquist clause then binds and no antialiasing
filter can be flat to Nyquist while being 100 dB down above it — there is no room left for a
transition band. `SampleRate::new` accepts any nonzero rate, so such a pair is constructible even
though no standard session or `.nam` file produces one. Both tags are `trace-partial:` naming that
region, and the clause needs an FRS decision (an explicit lower bound on the rates the bar applies
to, most likely) rather than more test coverage.
...
**Decision D-9.9 (2026-09-07)** — FR-IR-090's "stated target" is **unity broadband power**:
`namir_ir::PreparedIr` measures the gain that would bring the loaded taps to `sum(h^2) / channels
== 1`, clamped to +/-40 dB, and the engine's `Ir` stage applies it per slot behind a stepped
`ir.normalize_enabled`, **default on**.

*Rationale:* The requirement asks for a target and does not pick one. Unity power is the target
under which swapping cabinets changes tone and not level, which is the requirement's own stated
purpose; a peak-based target would leave a sparse IR and a dense one at wildly different loudness,
and an absolute LUFS target of the kind FR-NAM-090 uses is meaningless for a filter that is only
ever heard convolved with something else. The clamp is a trust-boundary bound, not a taste
judgement: an IR file is untrusted input, a near-silent one has an arbitrarily small `sum(h^2)`,
and the honest answer past +/-40 dB is to leave FR-IR-070's level control in charge rather than
amplify whatever noise the file does contain.

*Consequence:* The split mirrors FR-IR-070's — `namir-ir` measures and never applies, so the taps
it returns are still exactly the gain the source WAV encodes, and every policy question (on/off,
when to ramp) stays in the stage. The gain lives on the slot rather than folding into the stage's
existing per-channel level ramps, which would have been the smaller change: during a handover the
two slots are convolved separately and blended, so a single post-blend gain would apply the
incoming IR's correction to the outgoing IR too — reintroducing, inside the crossfade window,
exactly the level jump this decision removes. The slot's ramp is seated settled on its first block
rather than ramping in from unity, because a brand-new slot has no prior output to click against
and ramping in only slides its level by the whole correction across the first 25 ms of its own
fade.

*Measured, on the fixture that prompted this:* `ir_overlong_12s.wav` (12 s of noise at a
one-second decay constant) carries `sum(h^2) ~ 8000`, about +39 dB of broadband gain against the
~+11 dB of the ordinary library IR fixtures — roughly 28 dB hotter, which drove FR-CHAIN-090's
0 dBFS clamp (`chain.rs`) into audible hard clipping on every note. There is no limiter in the
chain (FR-OUT-030 is a Should and out of scope), so that clamp is a clipper, and nothing between
the convolver and it was ever going to make the level sane.
...
*The defect underneath the symptom, found in review of this change and fixed with it.* The
annihilation described above is not a property of the gate's position but of what the gate did:
`GateStage` gated channel 0 and **copied the result over every other channel**. That is one way to
apply one gate and it is strictly more than FR-CHAIN-050 asks — the requirement is that the *core*
process a single channel, and the stage that establishes that invariant is Trim, not Gate. The stage
now derives a single gain curve from channel 0 (`namir_dsp::NoiseGate::compute_gains`, added for
this) and multiplies every channel by it: one detector, one state machine, one envelope, nothing
overwritten. Channel 0's own result is bit-identical to what `NoiseGate::process` produced, asserted
in `namir-dsp`'s own tests, so no single-channel behaviour moves. **This is independent of the
ordering above and is why both changes ship together**: with the copy gone, FR-CHAIN-060's
`2 ch summed` is reachable under either order, so the reversal is not load-bearing for it and no
future reordering can restore the old behaviour by itself. It also narrows what a per-channel-gate
proposal has to build — see the interaction note in this pass's pull request.
...
*Consequence (added M15, 2026-09-07, issue #172) — `SlimmableContainer` supported; deferred list goes from four items to three.* The deferral assumed the container format was optional for users. Tone3000 exports transitioned to serving A2 models exclusively inside `SlimmableContainer` wrappers (63 of 63 sampled uploads in issue #172), making refusal of the wrapper a barrier to using supported A2 models. `NeuralAmpModelerCore` implements `SlimmableContainer` by selecting submodels against `max_value` thresholds, defaulting to the full-width (last) submodel when no size is requested (`NAM/container.cpp`). Namir loads the container by validating submodel ordering, requiring the terminal `max_value >= 1.0` and matching sample rates, and preparing the last submodel through `model::load`. The remaining deferred items (`condition_dsp`, FiLM conditioning, `.namb`) remain out of scope.

*Consequence (added 2026-09-07, issue #172, second review round) — the invariants above were checked against the reference, and one of them was stricter than it.* PR #177's review asked whether the ascending-`max_value`, terminal-`>= 1.0` and sample-rate checks reject files `NAM/container.cpp` accepts, noting that #169 and #170 had just corrected exactly that failure twice over, and that the citation could not be checked from this repository because the reference is not vendored. It is, however, checked out on the machine these parity runs use, at the pinned commit they name, so the question was settled by reading it rather than by argument — which is the same move that settled the prewarm premise at PR #174. **The first two invariants are the reference's own, verbatim**, including `<=` for strictness and `< 1.0` for the terminal test (`container.cpp:26-33`). **The third was not.** The reference compares each submodel against the *container's* expected rate and skips the comparison whenever either side is `NAM_UNKNOWN_EXPECTED_SAMPLE_RATE` — which is what an absent `sample_rate` key becomes (`get_dsp.cpp:280-286`) — so it never compares submodels to each other. This crate did, and therefore refused a container that declares no top-level rate and holds submodels at differing rates, which the reference loads. Corrected to mirror the reference; one zero-rate rejection is kept as a deliberate, narrow divergence, recorded at the check rather than left to be rediscovered. **The reviewer's specific worry about `max_value` did not survive contact with the source and is recorded as answered**: `[1.0, 2.0]` selects index 1 in the reference too (`container.h`'s "up to but not including", `container.cpp:83-95`), so validation and selection do not encode competing readings — and namir consults no threshold at all, taking the last submodel, which is the reference's own constructor default. What remains genuinely unestablished is what `max_value` *means* to a host choosing a size, which issue #172 asked and `container.cpp` alone does not answer; nothing in this path consumes it beyond ordering, so it is not load-bearing here.

---

### 9.6 Model prewarm — added 2026-09-07 (issue #173)

This subsection belongs to §9.1's NAM stage, appended here for the same reason §9.5 was: the
decision numbering stays monotonic and no existing text moves.

**Decision D-9.13 (2026-09-07)** — Silent prewarm is **part of matching the reference under
FR-NAM-030**, not an optional host convenience, and Namir performs it on every model it loads.
`PreparedNam::prewarm_samples()` mirrors `NeuralAmpModelerCore`'s `GetPrewarmSamples()` per
architecture — WaveNet's receptive field (`NAM/wavenet/model.h:71`, computed at
`model.cpp:616-620`), LSTM's fixed half second (`NAM/lstm.cpp:127-134`) — and
`PreparedNam::new_state_prewarmed()` applies it. It runs in **D-8.1's prepare step**, in
`namir-engine`'s `NamSlot::new`, on a worker thread.

*Rationale:* Three reasons, in order of weight. **(a) It is what the requirement already says.**
FR-NAM-030 asks that Namir's inference "match the reference NAM implementation" over a specified
signal; the reference's `DSP::prewarm` runs before that signal's first sample, so a comparison from
sample 0 against an un-prewarmed Namir is not a comparison of the same thing. No requirement text
changes here — this decision records the reading, which is exactly what the issue asked for rather
than assumed. **(b) The "host convenience" framing this project previously used undersells it.**
Prewarming is applied by the reference's DSP wrapper rather than by its inference kernel, which is
true and was the basis for `namir-nam`'s two comments calling it a caller-side recommendation and
declining to implement it. But *every* host using the reference gets that wrapper, so a settled
model is what a model's author heard when they exported it; a zeroed history is a state no listener
has ever heard, and "the model's declared initial state" is a defensible reading of the file that
is not a defensible reading of the sound. **(c) The cost was measured, not assumed.** Across ten
submodels from five real trainer-produced A2 exports and three real A1 models, the un-prewarmed
error is -29.2 to -58.3 dB over the first <=4096 samples (<=85 ms) and -104.9 to -138.2 dB after —
so the entire discrepancy between Namir and the reference on real files was the missing warm-up
(`docs/manual-tests/fr-nam-030-real-a2-models.md`).

*Rationale (placement):* Prewarming is thousands of samples of inference — ~6 347 for the real A2
shapes, 24 000 for a 48 kHz LSTM — which NFR-RT-010 forbids on the audio thread outright, and which
D-8.1's install step (3) has no room for either, since install is deliberately nothing but a
pointer move. Prepare is the step that already allocates, already runs on a worker, and already
holds the fresh `NamState`, so the warm-up costs the audio thread exactly nothing. It is placed in
`NamSlot::new` rather than one level up in `Command::load_nam` because `NamStage::load_model`
builds a slot without going through a command, and both routes must warm — the same
one-funnel argument D-8.1 makes for the slot itself.

*Consequence — load gets measurably slower, by about one percent of the budget it has.* Prewarming
is real inference and its cost was measured rather than waved past: on the committed golden models
in a release build, 5.4 ms (A2-Full), 1.1 ms (A2-Lite), 2.4 ms (a 48 kHz LSTM, which prewarms
longest at 24 000 samples) and 83 µs (A1-Nano). NFR-PERF-050's ceiling is 500 ms, so this spends
roughly 1 % of it at the top end and the requirement is not at risk; `namir-worker`'s
`resource_load` bench is where a regression would show. These are dev-machine figures and
informational only under §2's pinned-reference-machine rule — they are quoted here to establish an
order of magnitude, which is all the decision turns on, and none of them is offered as a certified
number.

*Consequence — a model swap now crossfades into a settled model.* D-8.1's handover means a user
changing models mid-performance previously crossfaded *into* a model spending its first ~85 ms
settling. Whether that was perceptible was never established and is now moot; it is not a listening
question this project has to answer.

*Consequence — the WaveNet count may exceed the reference's by a few samples, and that is exact
rather than approximate.* `prewarm_samples()` sums the head term over every layer array where the
reference adds one post-stack head term; it can therefore only over-estimate. For a WaveNet an
over-estimate is *equivalent*, not merely close: the network's whole state is convolution histories
that each span at most the receptive field, so once that many zeros have passed, further zeros
change nothing — asserted bit-exactly by
`crates/namir-nam/tests/prewarm.rs`'s settling test. For an LSTM the reference's own half second is
reproduced rather than improved on, because the target is what the reference does.

*Consequence — the generated goldens do not move, and that is a property of the fixtures rather
than evidence prewarm does not matter.* Every bias `namir-fixtures` emits is exactly zero, so
silence propagates as exact zeros and the settled state *is* the zero state; the committed figures
(-137.06 dB WaveNet-A1, -132.58 A2-Full, -126.46 A2-Lite) are unchanged after the goldens were
moved onto the prewarming path, which is the verification that the change is inert *there*, not an
argument that it is inert. Real models have non-zero trained biases and do move.

*Consequence — `NamStage::reset` still cannot re-warm.* Clearing inference state on the audio
thread was already impossible without allocating (`namir-nam` exposes no state reset;
`crates/namir-engine/src/stages/nam.rs`'s reset path records this as a known gap), and prewarming
does not change that: a hypothetical future RT reset would have to leave the state settled rather
than zeroed, which is a further reason such a reset cannot be a memset. Recorded so the gap's
description stays accurate rather than being discovered again.

*Consequence — prewarm needs an NFR-SEC-020 ceiling of its own, and this is a new class of bound.*
Every existing dimension ceiling in `namir-nam` bounds *memory or weight count*; prewarming is the
first thing that turns a declared shape into **work performed at load time**, and none of those
ceilings bounds it. Two files that pass every existing check can demand an unbounded load: a
WaveNet whose receptive field is the product of `MAX_LAYER_ARRAYS` x
`MAX_DILATIONS_PER_LAYER_ARRAY` x `MAX_KERNEL_SIZE` x `MAX_DILATION` (~1e11 samples, reachable at
roughly 18 M weights, comfortably under `MAX_TOTAL_WEIGHTS`), and an LSTM declaring
`sample_rate: 4294967295`, which is validated only as nonzero and asks for ~2.1 G samples. Either
hangs a load forever on a file that parses cleanly. `MAX_PREWARM_SAMPLES = 1 << 18` (~5.5 s at
48 kHz) bounds it, against a real maximum of 6 347 samples for the largest architecture this crate
runs and 192 000 for an LSTM at a 384 kHz model rate, and is exercised through a real hostile file
in `crates/namir-nam/tests/prewarm.rs` rather than asserted against itself. **The bound is relative,
and stating it that way is the point**: a sample count alone does not bound work, since a
pathological model can be arbitrarily expensive per sample, so what this ceiling actually
guarantees is that *loading a model costs at most what playing ~5.5 s of audio through that same
model costs*. That is sufficient because a model too expensive to prewarm inside that budget is one
NFR-PERF-010 already makes unusable for playback — the file-level hazard is bounded by the model's
own inherent cost, which the existing dimension ceilings do bound. It is also why the constant is
not a rounder, larger number: every doubling multiplies the same worst case. The clamp is applied at
`PreparedNam` only; each architecture's own accessor stays an unclamped transcription of the
reference's formula, so the tests that pin those against the reference pin the formula and not the
ceiling.

*Rejected:* prewarming at install time on the audio thread (NFR-RT-010, and D-8.1 exists to prevent
exactly this); prewarming inside `ResourceCache::get_or_load_nam` (the cached `Arc<PreparedNam>` is
shared and immutable — the thing that warms is the per-slot `NamState`, which does not exist yet at
that point); and deriving the count in the *engine* rather than in `namir-nam` (the receptive field
is computed from `Conv1D` fields private to `wavenet.rs`, and D-5.1 puts model knowledge in the
model crate).
...
*Consequence (added M15, 2026-09-06)* — Saving over an existing preset is guarded by confirmation across
...
*Consequence (added M15, 2026-09-06)* — Configured library roots are persisted in
...
*Consequence (added 2026-09-08, from issue #167):* **FR-IO-080's persistence behaviour preserves a
user-requested buffer size rather than overwriting it with fallback values, and clean installs leave
`buffer_size_frames` absent.** Previously, `persist_negotiated_audio` unconditionally wrote the
negotiated buffer size back to disk, which both overwrote user preferences when a device declined a
size and caused clean-install fallback values to permanently masquerade as explicit requests on
subsequent launches with different audio interfaces. On a clean install where no buffer size was
specified, `buffer_size_frames` is left absent (`None`) so the backend continues using the device
default, and `BUFFER_SIZE_DECLINED` (`app.audio_io.buffer_size_declined`) warns non-modally whenever
negotiation uses a fallback size differing from a requested setting. Furthermore, `AppSettings` and
`ChannelMapping` adopt `#[serde(default)]` to allow hand-edited partial settings files to deserialize
cleanly to their defaults; the explicit trade-off is that truncated-yet-valid JSON (such as `{}`)
silently defaults without routing through the corrupt-file recovery path or triggering
`SETTINGS_UNREADABLE`.

*What this costs, booked here rather than left to be discovered:* FR-IO-080 lists buffer size among
the four things that "shall persist between sessions", and after this change a buffer size **nobody
asked for** does not. A launch that negotiated 480 frames on a clean install writes no
`buffer_size_frames`, so the next launch renegotiates from the device's own ranges rather than from
that figure. That is the intended reading — the requirement is about the user's configuration
surviving, and a fallback the user never chose is not configuration — but it does mean the field is
absent from a settings file until something sets it, and
`docs/manual-tests/fr-io-080-settings-persistence.md`'s step 3 was amended to say so, its executed
transcript having recorded the old behaviour.

*One residue this cannot fix, recorded rather than papered over:* a settings file written by a build
**before** this change already carries a `buffer_size_frames` the old code auto-persisted, and
nothing on disk distinguishes it from a request. Such a file keeps that value, and a device that
declines it raises `app.audio_io.buffer_size_declined` naming a size its owner never chose — once,
until they set one. No migration is possible (the field is the only record and it is ambiguous by
construction) and none is attempted; the alternative, dropping every existing `buffer_size_frames`
on first load, would discard genuine hand edits to fix a cosmetic notice.

**Decision D-13.3 (2026-09-07, from issue #166)** — **The output stream asks the device for its own
buffer** (`cpal::BufferSize::Default`), never a size Namir picked. The negotiated buffer size still
sizes the engine's block (`audio_io::block_frames`) and the input stream; it no longer sets the
output device's period.

*Rationale:* One number was doing two jobs, and for the output stream the second job was harmful.
Measured on §2's reference interface (AudioBox 22VSL, 48 kHz, WASAPI **exclusive** mode) with a
synthetic callback cost injected into an empty chain — no IR, no model, so the figures are about
this mechanism alone: at a forced 10 ms period a 900 µs callback produced a **10.934 ms** measured
period, i.e. the slip *is* the callback duration to three decimals, and 12 224 captured frames were
dropped; at the device's own 3 ms period the same 900 µs callback slipped only 125 µs, and at the
~285 µs a real chain costs per callback there, the period held at **3.000 ms** with nothing dropped.
The slip fills `namir-app`'s bridge ring until it refuses captured frames — heard as crackling,
counted as FR-IO-060 xruns, and not a DSP deadline problem at all: the callback never once exceeded
its period in any run.

*Consequence:* **FR-IO-040's "select … buffer size" no longer reaches the output device.** The
selection keeps its latency and CPU meaning through the engine block and the input stream, but this
decision does not claim to satisfy that Must as literally written, and the gap is booked here rather
than left for a reader to discover. `crate::latency::estimate_round_trip` takes `Option` for the
output term and reports a **lower bound** when the device chose, because `cpal` exposes no portable
way to read back what it chose — the alternative was printing the requested size, a number that
reads as authoritative and would have been wrong by 336 frames on this device.

*What is not claimed:* why a device's own period carries headroom and a forced one does not was not
established, only that it does, repeatably, on this interface. The measurement is one device in
exclusive mode; shared mode, ALSA and CoreAudio are unmeasured. The rule is unconditional anyway —
"do not force a period the device did not choose" is the same argument in every mode, and `Default`
is the ordinary request on every backend, `Fixed` always the aggressive one.
...
*Consequence (added M15, 2026-09-06)* — `UiIntent` was extended with `AddLibraryRoot` and
`RemoveLibraryRoot` variants, bridging user gestures to background worker tasks without blocking UI
...
**Decision D-15.4 (added 2026-09-09, issue #35)** — **Accessibility support (FR-UI-030) is scoped to Windows and macOS.** On Linux (X11), accessibility events and AccessKit tree updates are a compiled no-op in the windowing stack (`baseview` and `egui-baseview`).

*Rationale:* AccessKit provides active platform adapters for Windows (`accesskit_windows` via UI Automation) and macOS (`accesskit_macos` via NSAccessibility). On Linux/X11, AT-SPI integration via AccessKit is not currently wired in `baseview` and requires DBus/AT-SPI daemon integration that is out of scope for the current windowing layer.

*Consequence:* Assistive technologies (screen readers like Narrator, NVDA, and VoiceOver) can inspect and navigate the user interface on Windows and macOS. On Linux, keyboard operation remains functional, but screen reader announcements are not delivered.
---
...
*Consequence (added M15, 2026-09-06)* — Added preset error codes in `namir-app` and `namir-clap`
...
(`.github/workflows/ci.yml:330`, `:367`) both build `-p namir-platform` on every push, and with no
...
per D-23.1's adjacency rule, is what moves the row off **UNRESOLVED** in `docs/05-test-plan.md`
...
| `baseview` (Namir fork) | 0.3.3 (`c0870cb`) | MIT OR Apache-2.0 | 2026-09-08 | Windowing; AccessKit integration | High — fork of low-adoption crate |
| `egui-baseview` (Namir fork) | 0.7.0 (`4a46c0a`) | MIT OR Apache-2.0 | 2026-09-08 | egui windowing embedding | High — fork of low-adoption crate |
...
| `rtrb` | 0.3.5 | MIT OR Apache-2.0 | published 2026-04-26; verified 2026-08-06; **bumped 0.3.4 -> 0.3.5, 2026-09-06** for RUSTSEC-2026-0274 (double free / use-after-free in `ReadChunk::commit` when an element's `Drop` panics). A patch bump, no API change; reachability here was not analysed -- the return ring carries owned resources whose `Drop` runs, so the bump was taken rather than the advisory triaged | D-7.2's SPSC command ring and D-8.1's return ring, in `namir-engine`. **A normal dependency — this one ships.** | Low — **zero** transitive dependencies, no build script, `no_std`-capable pure Rust, `rust-version = "1.38"` |
...
gated in **two halves with different flip dates**. The plan-diff half — `docs/05-test-plan.md`
...
coverage_clean`, `xtask/src/main.rs:343`) and CI's single invocation carries a single
...
(`xtask/src/main.rs:343`) and the argument parser recognises only `--write` (`:367-368`), so an
...
regenerated `docs/05-test-plan.md` itself, which no one may hand-edit. Landing the CI edit ahead of
...
generated plan (`docs/05-test-plan.md:20`, `:21`, `:24`, `:25`, `:27`, `:28`). They are six of the
...
annotated source coverage today (`docs/05-test-plan.md`: 28 rows carry `I`, seven of them
...
*Consequence (added M15, 2026-08-28, from issue #34 — a manual-test document now has to say whether
it was run, and `xtask traceability` reads that rather than the file name).* This decision makes a
`Verify: M` Must's manual document its **entire** traced artifact, and `check_partial_verify_code`
refuses a `// trace-partial:` naming such a Must — so between "no document" and "fully covered"
there was no disposition the tool could express, and a document existing was the whole of the test.
It credited `**Result: NOT EXECUTED**` and `**Result: FAIL**` exactly as it credited a clean pass:
the gate printed `clean -- all 130 Must requirements are covered` while six of those Musts' scripts
recorded no pass at all. The *Rejected — teaching `xtask traceability` to accept a manual-test
document as coverage for `Verify: I`* argument above already named the mechanism in as many words
("it cannot check whether a document's claim to have been executed is true"); it was true of the
`M` arm too, where the document is not a proposal but the standing rule.

**The verdict convention.** Every file under `docs/manual-tests/` — not only the ones a `Verify: M`
Must resolves to — must carry at least one **verdict line**: a line beginning `**Result:` whose
first words are one of exactly four upper-case tokens, `PASS`, `FAIL`, `PARTIAL` or `NOT EXECUTED`,
followed by whatever prose the author wants. Only `PASS` credits a requirement. Where a document
carries several verdict lines the least favourable wins, so a later run recording one executed step
cannot promote a script that is otherwise unexecuted. The convention is documented for its authors
in `docs/manual-tests/README.md`; `VERDICT_TOKENS` in `xtask/src/traceability.rs` is the only
definition the tool reads.

*Why a missing or malformed verdict is a hard error, not a gap.* A document with no verdict line, a
verdict line opening with something other than a token, or a `PASS` contradicted by its own sentence
aborts the whole `traceability` run, upstream of `--write`, of `--allow-uncovered` and of every
exit-status term — the same standing D-23.1's malformed annotations have, for the same reason: it is
a bad **input**, not a coverage gap. Left as a gap it would be relaxable by a flag; left as a credit
it would be the defect this note closes. The pre-M15 parser downgraded a self-contradicting `PASS`
silently, which got the outcome right and told the author nothing about which half of their sentence
won.

*Why the check covers supplementary documents too.* For any code other than `M` the document is
supplementary evidence and the gate never reads its verdict for coverage — but its executed-ness is
exactly as easy to misread by a human, and the `Verify:` code of a requirement is not frozen (FRS
§1.5 freezes identifiers, as the *Rejected — amending the `Verify` code* argument above records).
Eight of the twenty-six live documents carried no verdict line when this landed, five of them
supplementary documents for Musts. Only `README.md` is exempt, by exact name, because it is the
convention rather than a run.

*What this does not change.* No requirement became less met, and no verdict was promoted to keep a
build green: the six Musts this makes visible — FR-IO-030, FR-IO-050, FR-UI-030, FR-UI-040,
FR-UI-050 and FR-UI-070 — were already in exactly this state, recorded in their own documents in
plain English. D-18.5's split is untouched: they are uncovered Musts, which is the half
`--allow-uncovered` relaxes and which becomes required at the flip.

*Consequence (added M14, 2026-09-08, from issue #34's second half — worst-verdict-wins now holds
**across** a requirement's documents, not only within one).* The note above built the fold in the
wrong place to be complete: `parse_manual_verdict` folds a document's own verdict lines, but
`build_report`'s `'M'` arm found its document with `manual_test_docs.iter().find(..)`, so across
files it was first-match-wins. A requirement is matched by filename prefix **or** by a document's
`**Requirement (literal…):**` declaration block, and more than one document matching is now the
ordinary case rather than a curiosity — so the first match decided, and which document that was
depended on directory order. PR #159 shipped the audio-device panel with two scripts written for
the surface it had just built, `fr-io-010-device-selection.md` and
`fr-io-040-sample-rate-and-buffer-size.md`, both `NOT EXECUTED`; both were invisible, because
`fr-io-010-device-enumeration.md` sorts first, declares both ids and records a `PASS` earned
against the pre-panel surface. FR-IO-010 and FR-IO-040 therefore read plainly covered in
`docs/05-test-plan.md` on the strength of a script that does not exercise what the panel added —
the same defect this note opened against the document-exists-so-it-passed rule, one level up.
**Every** matching document is now read and the least favourable verdict decides; one non-pass
blocks the requirement and is the document the plan names, whichever sibling passed. Uncovered
Musts go 4 → 6, the two new ones being exactly those. **What this does not change**, on the same
footing as the paragraph above: no requirement became less met, no verdict was promoted, and §14's
`5.11 IO` bullet already counted both as Partial — the ledger was stricter than the gate, which is
the direction that costs nothing to find out. The practical rule for authors, stated in
`docs/manual-tests/README.md`: adding a script for a requirement takes it uncovered until the
script is run, and that is intended.
...
--workspace`, and `ci.yml:322` runs `cargo build --release -p namir-clap --lib`. §22 gains **R-17**.
...
*Consequence (added M14, 2026-09-06)* — **GUI API negotiation and parent window handling widened
...
| R-9 | ~~A2's flat weight-layout order must be re-derived from `NeuralAmpModelerCore` (`NAM/wavenet/detail.h`, `NAM/wavenet/params.h`) the same way A1's was, and A1's was then proven correct to −131 dB by S-1's cross-implementation parity. A silently-wrong order is the dangerous outcome, not a loud one: the model loads without error, runs at the expected cost, and produces plausible-sounding output that is wrong — and no amount of listening will detect it, which is exactly the failure mode NFR-QUAL-030 exists to forbid. Grouped and bottleneck convolutions, per-layer variable kernel sizes and the 1×1 projections all multiply the number of ways the order can be subtly wrong relative to A1.~~ **RETIRED at M10, 2026-08-09.** Mitigated by process, not only by a passing test: two agents derived the weight order independently from the upstream C++ source, neither reading the other's code — `crates/namir-nam`'s own A2 implementation, and `crates/namir-fixtures`'s A2 generator plus `reference_infer_a2`. Their independently-derived orders agree exactly, and the in-tree Rust-vs-Rust parity test (`crates/namir-nam/tests/a2_fixtures.rs`) agrees to the bit, `-inf` dB — core A2's `LeakyReLU` has no rounding-sensitive transcendental function, so bit-exact agreement between two independent implementations is the strongest evidence available that neither has a subtle porting bug. Cross-checked a third way, against the actual reference this row named: a real `NeuralAmpModelerCore` render (pinned `3cde95c`, `-DNAM_USE_INLINE_GEMM -DNAM_ENABLE_A2_FAST=OFF`) measured A2 Full/A2 Lite at -90.31 dB each (`WaveNetShape::Standard`-scale synthetic fixtures) and, on smaller golden-reference fixtures committed for FR-NAM-030 (`crates/namir-nam/tests/golden_reference.rs`), -137 dB — comfortably inside FR-NAM-030's -90 dB floor either way, and nowhere near the near-zero-correlation signature a genuinely wrong weight order would produce. None of grouped convolution, per-layer variable kernel size interacting with groups, or the 1×1 projections this row worried about compounding turned out to need building: core A2's own shape requires `groups == 1` throughout, so that multiplicative risk never existed in the shipped scope. | ~~Retired 2026-08-09~~; reopened 2026-08-12; **Medium** (narrowed, M14 Phase 4b, 2026-08-12) | Retired: independent double implementation plus a real-reference cross-check, both measured; see M10's roadmap close-out (`docs/03-implementation-roadmap.md` §17) for the full figures. **REOPENED M14, 2026-08-12 — the real-reference leg cannot be reproduced, and it is the only leg that addressed this row's actual concern.** R-9 is about A2's flat weight-layout order being re-derived, and a wrong order being *silent*. Bit-exact agreement between two in-house Rust ports proves they derived the **same** order, not the **right** one, so the double-implementation leg cannot substitute. The third leg — "A2 Full and A2 Lite measured -90.31 dB each against a real `NeuralAmpModelerCore` render" — has **no committed artifact** (`crates/namir-nam/tests/golden/` holds `wavenet_nano.nam` and `lstm_tiny.nam` and no A2 model), **no tool that could regenerate it** (`xtask` has no fixture-writing subcommand; `xtask nam-parity` is architecture-agnostic but every recorded run is LSTM-only), and **no manual-test write-up** (LSTM has `docs/manual-tests/fr-nam-020-real-lstm-models.md`; A2 has no equivalent). Worse, the two records of it disagree: this roadmap passage attributes the figures to **A2 Full and A2 Lite**, while `crates/namir-nam/tests/golden_reference.rs` restates the same measurement — same -90.3 to -90.9 range, same "flat regardless of signal length or complexity", same non-associativity explanation — as a **`WaveNetShape::Standard`** run, which is an A1-only enum. Two structurally different models (channels 8 vs 3, different layer counts and weights) reporting an identical figure to two decimal places is also not what independent measurements normally look like. Neither record is safe to rely on. **Mitigation, and the only thing that retires this row again:** commit an A2 golden — a generated A2-Full (and ideally A2-Lite) fixture rendered through the pinned reference build over `tests/golden/input_10s.wav`, asserted in-process beside the two existing golden tests, with a probe longer than A2's ~6300-sample receptive field. Tracked as issue #37 and owned by **M14**. **NARROWED, NOT RETIRED — M14 Phase 4b, 2026-08-12.** The mitigation this row demanded is built and committed: `crates/namir-nam/tests/golden/a2_full.nam` and `a2_lite.nam` (generated, D-19.1, seed 30, regenerable in-tree by `cargo test -p namir-nam --test golden_reference regenerate_the_a2_golden_models -- --ignored` and guarded against drift by `the_a2_golden_models_match_their_generator`), each rendered through the pinned reference build — `sdatkinson/NeuralAmpModelerCore` at `3cde95c`, `-DNAM_USE_INLINE_GEMM -DNAM_ENABLE_A2_FAST=OFF`, built outside the repository — over `tests/golden/input_10s.wav`, and asserted in-process by `a2_full_matches_the_real_reference_implementation` and its `Lite` sibling. **The figures: A2-Full -132.58 dB, A2-Lite -126.46 dB**, against a bar this pass tightened from -85 dB to FR-NAM-030's own -90 dB so that a plain tag asserts the requirement's own threshold. The probe is 480 000 samples against a 6 346-sample receptive field, so ~98.7 % of the compared output is settled rather than startup transient — the specific defect this row's mitigation clause called out. FR-NAM-030's and FR-NAM-150's tags are promoted to plain `trace:` on the strength of it. The reopening paragraph's third complaint — "no tool that could regenerate it… every recorded `xtask nam-parity` run is LSTM-only" — is answered on both counts: the `.nam` half regenerates in-tree from the `#[ignore]`d recipe above, and `xtask nam-parity` reproduces both figures from a second code path (`--model crates/namir-nam/tests/golden/a2_full.nam --input crates/namir-nam/tests/golden/input_10s.wav --reference crates/namir-nam/tests/golden/a2_full_reference.wav` prints -132.58 dB; the `a2_lite` triple prints -126.46 dB), so this is the register's first recorded A2 `nam-parity` run. **Three things this settles, and three it does not.** *Settled:* the weight order this row is about is now cross-checked against the reference implementation itself, not only between two in-house ports, over settled output, at full scale, reproducibly, with the artifact committed and the recipe executable. *Also settled, and it corrects this row's own text:* the reopening paragraph above records M10 as having measured "A2 Full and A2 Lite at -90.31 dB each" and flags that two structurally different models reporting an identical figure to two decimal places is not what independent measurements look like. This pass measures them at **-132.58 and -126.46** — different from each other, as one would expect, and both ~40 dB below the recorded figure. The M10 number was therefore not an A2 measurement; the competing record in `golden_reference.rs` attributing that same -90.3 range to a `WaveNetShape::Standard` (A1-only) run is the one consistent with what is now measurable. That is a resolved contradiction, recorded rather than quietly dropped. *Not settled, and each is why this row stays open rather than being retired a second time:* **(a) no genuine trainer-produced A2 export has ever been loaded.** `namir-fixtures`' generator, `namir-nam`'s parser and this comparison's target all derive the A2 schema from readings of the same C++, so a *shared misreading* of what a real exporter emits is invisible to every test in the tree, including this one; `file.rs`'s A2 fields are `#[serde(default)] Option<_>` with no `deny_unknown_fields`, so a real file carrying a feature under an unanticipated key is silently ignored rather than rejected. Closing this needs a real export, not a render. **(b) The fast path is not compared.** `NAM_ENABLE_A2_FAST` defaults **ON** upstream (`CMakeLists.txt:58`) and `a2_fast.cpp`'s `is_a2_shape` matches exactly the two shapes FR-NAM-150 names, so a default-built host runs real A2 models through a hand-written kernel this comparison switches off. The rationale for switching it off is now *recorded* (at `golden_reference.rs`'s header: the general path is the definition and the fast path an optimisation of it, and pinning `NAM_USE_INLINE_GEMM` for reproducibility while leaving a second hand-written kernel free would defeat the pin) where before it had none — but recording a rationale is not taking a measurement, and no measurement of `a2_fast.cpp` against anything was taken here. **(c) `A2Shape::BottleneckProbe` has no golden**, deliberately: `is_a2_shape` rejects it by construction, so no reference build will run it. It keeps its in-house cross-check in `tests/a2_fixtures.rs`. **Severity is reduced from High to Medium** on (a) alone: the silent-wrong-order failure this row was raised about is now excluded by a real-reference comparison, and what remains is a schema-fidelity question about files this project has never seen, whose failure mode is a model that is *rejected or mis-parsed on load* rather than one that runs plausibly and wrong. **NARROWED AGAIN — 2026-09-07, PR #174: (a) is closed, and its predicted failure mode was half right.** Ten submodels from five trainer-produced Tone3000 exports were rendered through the same pinned build and compared by `xtask nam-parity`, at **-104.96 to -124.73 dB** against FR-NAM-030's -90 dB floor, with a three-model real A1 control at -137 to -138 dB; written up in `docs/manual-tests/fr-nam-030-real-a2-models.md`. So the shared-misreading-of-the-schema risk (a) was raised about did **not** materialise in the arithmetic. It did materialise in the other half of (a)'s own sentence — "a real file carrying a feature under an unanticipated key is silently ignored rather than rejected" — inverted: `gating_mode` and `secondary_activation` were rejected on the *presence* of fields that are inert in every real export (all `"none"`, all `null`), so every one of the 126 submodels in the 63-file sample refused to load at all until #169 and #170 narrowed both checks to test the value. The register's own caution earned its keep, one step earlier in the pipeline than it predicted. Two figures this row states are unaffected and were re-verified rather than assumed: the committed goldens still print **-132.58 dB** (A2-Full) and **-126.46 dB** (A2-Lite) through the now-prewarming `nam-parity`, because every bias `namir-fixtures` emits is exactly zero and a prewarm on such a fixture is a numerical no-op. That fact also corrected two committed comments in `crates/namir-nam/tests/golden_reference.rs` which had asserted, wrongly, that `WaveNet` does not override `GetPrewarmSamples`; it does (`NAM/wavenet/model.h:71`), and the goldens' conclusion held for a different reason than the one recorded. **(b) and (c) are untouched and this row stays open on them**; severity stays **Medium**. |
| R-10 | **New, from M8-planning, 2026-08-08.** D-13.4's forked `cpal` is a maintained fork: it must be rebased as upstream moves, and every rebase is work that produces no user-visible benefit and can silently regress the Windows audio path. It is also the first non-registry dependency in the tree, which weakens the reproducibility NFR-SEC-040 (Should) wants and needs a named `cargo-deny` `[sources]` allowance that the default policy would otherwise reject. **Status note (added M11, 2026-08-11, from building it) — the rebase burden is higher than D-13.4 assumed, and the upstreaming exit is in better shape than this row's own wording implies.** *Burden.* The fork as pinned (`rev 2edbacb`) is seven commits on upstream trunk `e0893c3`: **2867 insertions / 144 deletions across 11 files**, not the few-dozen-line change the Mitigation column's "keep the fork's diff minimal" was written about. `src/platform/wasapi_ext.rs` (456 new lines) and `examples/wasapi_exclusive_probe.rs` (1443 new lines) are new files that can never conflict, but `src/host/wasapi/device.rs` changed 546 insertions against 130 deletions and **will** conflict. The cause is structural rather than carelessness: a per-call extension trait means threading the share mode through every enumeration, negotiation and stream-building path, and D-13.4's M11 note records why no smaller shape was available to this workspace. Severity stays **Medium** — the work is bounded and one file concentrates almost all of it — but budget a real rebase with a re-run of `docs/manual-tests/fr-io-020-wasapi-exclusive-mode.md`, not a mechanical replay; M11's own two defects were both invisible to every automated check, so a rebase that compiles and passes CI is not evidence that the Windows audio path still works. *Exit.* "Offer the change upstream; if accepted, the fork disappears" reads as a hope, and the evidence says it is better than that. Upstream wants this feature **and wants it in this shape**: issue **#1220** (v0.19 design goals, open) lists "Exclusive mode on CoreAudio, PipeWire & WASAPI (#106)" under working out extension traits, which is exactly what this fork implements. PR **#843** ("Added WASAPI exclusive host") is **not** evidence that upstream refused the feature, and should not be cited as such: its author closed it himself in November 2024 saying it needed updating and bug fixing, and **no maintainer ever reviewed it**. PR **#1195** (`AUDCLNT_STREAMOPTIONS_RAW`) is a different feature that establishes no reusable seam; it is still open, with two maintainers pushing back on its field-on-`StreamConfig` shape in favour of the same extension traits #1220 names. What the exit lacks is a date and an agreed API, not upstream willingness — so the mitigation to keep alive is offering the change, not preparing to carry the fork indefinitely. (Facts checked against the upstream repository on 2026-08-11; #1195 and #1220 were open on that date and may have moved since.) **Status note (added 2026-09-09, PR #188, issue #35) — reproduced here as an appended note; #188 wrote it *in place of* the M11 text above, and issue #200 restores that text and appends this beneath it per `AGENTS.md`'s convention:** risk now extends to three Namir-maintained forks (`cpal`, `baseview`, `egui-baseview`); the `cpal` fork is rebased onto the 0.19.0 trunk line (`381cf1d`) rather than the unreleased line the M11 pin took, its WASAPI diff is stated there as 2867 insertions / 144 deletions across seven files (`cpal-src/src/host/wasapi/`), and all three repositories are allowed via narrow exact-URL entries in `deny.toml`'s `allow-git`. **Correction (added 2026-09-11, issue #200):** two claims in that note do not hold. *(1) The pin.* It says all three are "strictly pinned by exact commit hash in manifests (`rev = \"...\"`)". `cpal` and `egui-baseview` are; **`baseview` is not.** It arrives transitively through the `egui-baseview` fork's own manifest as a **branch** spec and is pinned only by the lockfile, which resolves it as `source = "git+https://github.com/ErwanLegrand/baseview.git?branch=accesskit-design#c0870cb5b5279e4fff174ea8b407f19671e4fd5e"` (`Cargo.lock:163`), so any `cargo update` moves `baseview` to whatever `accesskit-design` then points at while the other two stay put — the exact "never by branch" case this row's Mitigation column warns about, and what `deny.toml`'s own comment already records. The remedy is one line in `ErwanLegrand/egui-baseview`'s own manifest — `rev = "c0870cb…"` in place of `branch = "accesskit-design"` — and it is **not takeable inside this repository**: a workspace `[patch]` entry cannot express it (cargo refuses a patch that resolves to the same source it patches), and a direct `rev`-pinned dependency edge on the same git URL conflicts with the transitive `branch` spec. (**"Conflicts" is corrected by the 2026-09-11 re-confirmation below: cargo does not stop you — it silently adds a second copy.**) It is therefore recorded as an open action on the fork, owned by issue #200, rather than silently fixed here. *(2) The diff figure.* Neither measurement supersedes the other: **2867 insertions / 144 deletions across 11 files** was measured 2026-08-11 against base `e0893c3` (the M11 pin `2edbacb`), and the seven-file figure is a different measurement against a different base (0.19.0, `381cf1d`). Both stand as dated measurements; §17's detailed M11 fork row carries the same statement. Separately, #188's 0.18.1 → 0.19.0 rebase *is* the rebase event this row warns "can silently regress the Windows audio path", and no manual re-run of `docs/manual-tests/fr-io-020-wasapi-exclusive-mode.md` is recorded against it. **Re-confirmed 2026-09-11 (issue #200):** the `[patch]` refusal stated above was reproduced rather than reasoned about -- `cargo generate-lockfile` aborts with ``error: patch for `baseview` points to the same source, but patches must point to different sources`` and writes no lockfile; a direct `rev` edge on the same URL adds a *second* `baseview v0.3.3` instead of repinning the transitive one. A patch pointing at a *different* source **is** accepted, including a git one: both a `path =` checkout and `{ git = "<a different URL>", rev = "c0870cb5…" }` against a mirror of the fork resolve and substitute for the branch source, the mirror leaving `baseview` a git dependency pinned by rev. So the claim narrows to **no route at the same git URL pins it**; a rev pin is reachable here, at the cost of a fourth exact URL in `deny.toml`'s `allow-git` (without it `cargo deny check sources` exits 8) and a mirror to maintain alongside the fork — the same shape as the Mitigation column's vendoring fallback, and worse value than the one-line `rev =` fix in `egui-baseview`'s own manifest. **Resolved 2026-09-13 (PR #225, issue #214)** — the one-line fix the 2026-09-11 notes said was "not takeable inside this repository" was taken inside the fork after all: `egui-baseview`'s own manifest now pins `baseview` by `rev = "c0870cb…"` (commit 672ea81, `4a46c0a`), so the lockfile no longer carries `?branch=accesskit-design` and `cargo update` can no longer move `baseview`. The `xtask git-sources` subcommand (added M15) enforces the `?rev=` requirement at the lockfile level and fails the build on any source without it. | Medium | Pin by commit hash, never by branch. Keep the fork's diff minimal — a share-mode parameter and its format-negotiation consequences, nothing else — so rebasing stays mechanical. Offer the change upstream; if accepted, the fork disappears and D-13.4 reverts to a version bump. Vendoring into the tree is the fallback if the pin proves insufficient for NFR-SEC-040. **Added M11, 2026-08-11:** the `[sources]` allowance this row names was, until M11, enforced by no CI step at all — `license-audit` ran `cargo deny check licenses` and `network-free` ran `check bans`, and `check sources` ran only when somebody typed the full `cargo deny check` locally. `license-audit` now runs it, so a *second*, unreviewed git source fails CI rather than passing unnoticed. **Added 2026-09-11 (issue #200):** the "never by branch" clause above is currently violated by `baseview` — see this row's 2026-09-11 correction. The fix is a one-line `rev` pin in the `egui-baseview` fork's own manifest and is tracked as an open action on that fork; this repository's manifests cannot express it. **Narrowed 2026-09-11 (issue #200):** not at the *same* git URL — a `[patch]` to a mirror URL, or vendoring, can express a rev pin, at the cost this row's correction states (a fourth `allow-git` entry and a second repository to keep in step). |
| R-11 | **New, from M8-planning, 2026-08-08.** Release binaries are unsigned on both signing-relevant platforms, and the two platforms fail differently. Windows: SmartScreen warns on every release until reputation accrues, which it never does for a low-volume unsigned publisher, and Smart App Control blocks unsigned binaries outright rather than warning. macOS is worse in kind, not degree: a quarantined **plugin** loaded by a DAW has **no user-visible "Open Anyway" path at all** — it simply fails to load — and macOS 15 removed the Control-click bypass that previously worked. macOS releases are therefore developer-only in practice until a signing identity exists. | Medium | D-18.3's signing-conditional structure: signing steps are skipped when the identity secret is absent and the unsigned build takes the identical code path, so enabling signing later is adding a secret, not restructuring. State the caveat in the release notes rather than letting users discover it. Revisit before any release aimed at non-developers. **Restated at M13, 2026-08-11, now that the pipeline exists and this row is about shipped artifacts rather than planned ones.** The structure is built and correct: `packaging/macos/make_installer.sh` reads its identities from the environment, does nothing when they are empty, and runs the identical sequence either way, with the caveat printed at the end of every unsigned run and shown in the installer's own welcome pane so it travels with the artifact. **But "adding a secret" understates what turning signing on costs, and this row should not have implied otherwise:** an identity *string* is not an identity on a CI runner — `codesign` needs the certificate imported into a keychain, i.e. a `.p12` secret plus `security create-keychain`/`import`, which is unbuilt. So the signed path is unreachable from CI today even with the identity secrets set, and macOS remains developer-only in practice for the reason this row gives, not merely for want of a purchase. Windows is unchanged and unmitigated: SmartScreen warns on every release, Smart App Control blocks outright, and the installer is unsigned. **Recorded as still open at M14, 2026-08-12 — deliberately not decided.** Issue #23 asks whether 1.0 is a public release, and that is the question this row actually turns on: if it is, a quarantined macOS plugin has no user-visible load path at all — not a warning, no load — and the CI signing route is unreachable besides, because the keychain import a `.p12` needs is unbuilt. M14's Phase 0 does not answer it, because "who is 1.0 for" is an owner's decision and the two answers differ by a purchased identity, a secret-handling design and a macOS notarisation step, none of which is documentation work. What M14 does settle is that **the signed path is not built in this milestone**, so whichever answer is taken, taking it is work that follows. Read this row as unchanged and unmitigated, not as having been considered and accepted. **Status note (added 2026-09-07, issue #23) — the decision is taken, and the CI half is built.** The owner has decided to obtain an **Apple Developer ID** for 1.0, and `release.yml`'s macOS job now has the keychain-import step the M13 restatement above said was unbuilt: a `NAMIR_APPLE_CERTIFICATE_P12_BASE64` gate imports the `.p12` into a temporary keychain (`import_cert`, with `security create-keychain`/`import`/`set-key-partition-list`), the existing `Package` step reads the identity and notary secrets unchanged, and an `always()` cleanup step scrubs the keychain afterwards. The `publish` job keeps the release notes honest by reading the macOS job's reported signing and notarization outputs (`signed`, `notarized`) rather than re-inferring from secrets: when signed and notarized, the notes say the macOS `.pkg` installers are signed, notarized and stapled, and explain verification (`spctl --assess`, `xcrun stapler validate`) and Gatekeeper; otherwise, the notes say macOS binaries are unsigned, while unconditionally reporting the Windows SmartScreen status. `packaging/macos/README.md` documents the seven secrets and the `.p12` export/encode commands. **What is not finished is the credentials side, not the pipeline side**: the signed path — keychain import through notarised, stapled artifacts on a real tag push — still needs the purchased identity, the `.p12` configured into repository secrets, and a first signed run to prove the sequence against Apple's actual tooling. The row stays open at **Medium** until that run exists; this change is what removes "the keychain import is unbuilt" from the reasons why. |
| R-12 | **New, from M9's P0 decision pass, 2026-08-08.** D-16.5's writer is synchronous: the thread that emits a record holds the logger mutex across one unbuffered `write_all`, and `namir-app`'s UI thread is one of the threads that emits records. FR-UI-060 budgets no frame above 100 ms while a 10 000-file library scan runs — which is also the condition under which `namir-worker`'s pool threads log most heavily, so the UI thread can queue behind a pool thread's disk write on a slow or contended volume. Not observed; what is recorded here is the shape of the interaction, before anyone builds it. | Low | Keep the default level at `info`, where records are per user action rather than per frame, and hold D-16.5's rule that no record is emitted from a per-frame path. **The detector this row wants does not quite exist, which is worth stating rather than assuming.** FR-UI-060's own timed check (`crates/namir-ui/src/library_view.rs:281-339`) renders the full library view against a real 10 000-entry corpus and asserts the 100 ms ceiling, so it would catch a per-frame regression in the view itself — but it builds its snapshot with `scan: None` (`:317`), so it never runs under FR-UI-060's own "while a scan is in progress" condition and would not see this interaction at all. Extending it to a scan-in-progress snapshot is the cheap way to make it the detector, and FR-UI-060 is one of the requirements M9a's quantifier sweep reaches on its own terms under D-23.1. If a regression does appear, the fix is additive and leaves D-16.5 intact: a bounded queue in front of the same writer, overwriting oldest with a count — the policy D-7.3 already applies to outbound telemetry. |
| R-13 | **New, from M9's P0 decision pass, 2026-08-08.** D-23.1's `trace-partial` is a laundering surface: it is strictly easier to write than the test that would close a gap, and a contributor under time pressure has a sanctioned way to make a half-met Must read as covered. The failure mode is quiet and cumulative rather than acute — no build breaks, no number moves, the gate stays green, and partials accrete until "PARTIAL" is the normal disposition and carries no information. This project has the precedent: §14's snapshot table went six rows untouched since M0 while reading as current, and M7's close-out claimed sixteen requirements were individually investigated when at least three were not. | Medium | Four mechanisms, none relying on diligence. (a) A partial costs *more* keystrokes than a full tag — the `uncovered:` line must name the specific member and a closing milestone in prose, in the diff. (b) It is rendered into generated, checked-in `docs/05-test-plan.md`, so it is visible in every PR and in `git log -p` permanently. (c) There is no path to 1.0 through a partial: D-18.5's zero-uncovered half becomes required at M13's close-out, and M8's exit checklist reads §14's table adjudicated under D-23.2, where a partial cannot be counted **Done**. (d) The ordinary run prints the partial count on every invocation, so the number is in front of whoever runs the gate rather than buried in a table. Re-read the partial list at each milestone close, the way §22's own rows are re-checked; if the count is not falling by M12, the mechanism is being used as a bypass and this row is not mitigated. **The condition above has fired, and this is the reinterpretation, written down rather than assumed (M14, 2026-08-12).** M12 and M13 have both run, M9b has run, and the partial count has gone **56 -> 68**, not down. A stated test that fires and is then explained away is worth nothing, so the explanation is recorded here at the row that made the prediction, with its arithmetic, and the row is **not** declared mitigated on the strength of it. *The arithmetic.* M9a's sweep left 130 Musts as 54 plain / 56 partial / 20 with no tag. The 20 were the number that mattered and they are now **zero**: M9b drove twelve uncovered Musts to covered in one milestone (FR-CFG-020, FR-NAM-060, FR-CLAP-030/-040/-070/-080/-100/-130, FR-ERR-010, NFR-PERF-040, and M13's two unclaimed handovers FR-UI-020 and FR-UI-070), and most of them landed as partials because that is the honest disposition for a newly built artifact that does not yet span its requirement's whole set. M9b then demoted **two of its own plain tags** — FR-CLAP-080 and FR-ERR-010, both written earlier in the same milestone — on finding they over-claimed. M14's own A2 pass demoted **two more**, FR-NAM-030 and FR-NAM-150, on finding that no A2 model has ever been compared against `NeuralAmpModelerCore` (see **R-9**, reopened). Every one of those movements makes the ledger say something *weaker* and *truer* than it said before. *Why that is not the failure this row predicted, and how to tell the difference.* The failure mode described above is a Must whose gap is real, whose closing test is not written, and whose partial is used to make it read covered — a substitute for work. Every rise counted here is the opposite: a requirement whose artifact was *built* in the same milestone that then declined to over-claim for it, or a plain tag retracted by the milestone that wrote it. **The discriminator is not the count's direction but who moves a tag and in which direction:** a partial written by the author of the test it annotates, demoting their own claim, is the mechanism working; a partial written to avoid writing a test is the bypass. This row's original test used the count because a count is cheap to check, and the count turns out to conflate the two. The count is retained as a printed figure — it is still the cheapest thing to look at — and it is **demoted from a test to an indicator**. *What replaces it as the test, since a risk row without one is decoration.* Two things, both already built and neither relying on diligence: **the uncovered-Must count**, now zero, whose rise is unambiguous because a Must can only become uncovered by someone deleting a tag; and **the `uncovered:` field being re-read against the tree at each milestone close** rather than carried forward, which M9b's own addendum demonstrates the value of by finding nine such fields asserting facts that were false when read, four of them falsified by M9b's own work. Neither can be satisfied by writing more prose. **This row stays open at Medium**, and the honest statement is that mitigations (a)-(d) are intact while its stated test is not: what it lacks is a cheap mechanical discriminator between the two kinds of partial, which nobody has yet designed. |
...
mechanism, including why `docs/05-test-plan.md` is generated rather than hand-authored.
...
`:135`. The result is checked in — `docs/05-test-plan.md:70` records FR-NAM-070's components as ``
...
- **An artifact exists but fails either question → `// trace-partial: <ID>`, immediately followed by
  ...
  (`crates/namir-worker/tests/rt_stress.rs:139-150`, whose own doc comment explains that it wants
...
- **Nothing covers it → no tag, in any form.** A requirement whose implementing milestone has not
  ...
  FR-PKG-010/-020/-030/-040 (M13) — **nine** of the twenty-four Musts `docs/05-test-plan.md`
...
`docs/05-test-plan.md` rather than hand-maintain "a document that would drift the moment a test
...
(`crates/namir-engine/benches/denormal_guard.rs:412`, `six_stage_chain.rs:242`,
...
confirmed by the generated plan recording exactly one component for each (`docs/05-test-plan.md:61`,
...
and a rejection here is a hard error that aborts the entire run (`xtask/src/main.rs:331-337`) — not
...
there is**, none appearing at any admissible site (`docs/05-test-plan.md:7`, `:9`, `:36`, `:37`,
...
prefix of neither marker spelling; the tightened `fn_name_embeds_id` (`:553-576`) kills the fn-name
class (that note's `:135` and `:338`, likewise). The tool's own tests say so by name
(`a_marker_that_does_not_begin_the_line_is_not_a_tag`, `:1347-1358`, whose two cases are labelled
"the `:317`/`:325` class" and "the `ci.yml:109` class"), and `check_adjacency`'s doc comment carries
the same correction (`:490-508`). What adjacency does buy is a fourth class none of those reach: a
...
Must count into the generated `docs/05-test-plan.md`, failing the build when §14's denominators or
...
`params.lock` and `docs/05-test-plan.md`. **Rewriting §14's existing table in place** — six
...
| 0.40 | 2026-09-07 | **D-9.8 is withdrawn; the chain is rebuilt to FR-CHAIN-010's original `trim → gate → nam → ir → eq → out`.** The decision text stands as history per this document's convention and gains a `*Consequence (added M15, 2026-09-07)*` note; FR-CHAIN-010's M9a amendment, which had rewritten the requirement to match the code, is superseded by a note of its own. **What M9a weighed twice was one argument — D-9.8's usability one — and the argument on the other side was in its own note, two paragraphs later, filed as an observation about FR-CHAIN-060's Stereo row and never carried back to the question it had just answered**: Gate is mono-core and copies channel 0 over every other channel, so placing it ahead of Trim, the chain's only cross-channel mixing, annihilated the right channel before the −6 dB sum could use it — `2 ch summed` was unreachable except as a side effect of disabling the gate. The trade is recorded as a trade: the gate's threshold now moves with the input trim, which is the whole cost D-9.8 was written to avoid and nothing has made it smaller; it buys a requirement and its code agreeing without either being amended to the other, for the first time since `875068e`, and a stereo input reaching the core as both channels. **AQ-2 is not reopened** — it asked for and got confirmation on 2026-08-04, which is a matter of record, not a live question. Moving with the decision: `build_default_chain`, three stage doc comments, `chain_probes.rs`'s transcription of the order and two of its five transposition probes (the Gate↔Nam pair needs the gate's envelope times inverted to be observable at all, and the doc comment says why), `namir-clap`'s stereo-pair test — which pinned L-only as the shipped reading of FR-CHAIN-060 and now pins the sum — and both benches' assembly order. No requirement changes priority; FR-CHAIN-070's absent chooser now falls back to the sum rather than the left channel, and FR-CHAIN-060 stays satisfied on the other of its two permitted options. Roadmap §6's M2 bullet gains a dated amendment rather than an edit. **Unrelated to the order, in the same commit**: `trim.gain_db` is displayed as "Input Level" (key and `ParamId` unchanged), and `namir-ui` moves both meters and both level controls into the top panel as two columns, each meter drawn beneath the control whose output it reads, with the bars filled in the brand mark's orange rather than `egui`'s default blue. **The copy underneath that symptom is fixed in the same pass, raised in review**: `GateStage` gated channel 0 and copied the result over every other channel, which is more than FR-CHAIN-050 asks of it — the *core* processes a single channel, and Trim is what establishes that invariant. It now derives one gain curve from channel 0 (`namir_dsp::NoiseGate::compute_gains`, added for this) and multiplies every channel by it: one detector, one state machine, nothing overwritten, channel 0 bit-identical to the old path. Independent of the ordering, which is why both ship together — `2 ch summed` is now reachable under either order, so the reversal is not load-bearing for it. `gate.rs`'s FR-CHAIN-050 test moves with it, from asserting that channel 1 carries channel 0's samples to asserting the three things the requirement actually wants: one shared gain curve, channel 0 alone deciding, and no channel's content replaced. |
| 0.41 | 2026-09-07 | **FR-IR-090 is implemented; new Decision D-9.9 states the target it was missing.** Reported as an over-long IR clipping on load, and the clipping half is exactly what `crates/namir-ir/src/lib.rs`'s own doc comment had said since M3 — "not implemented; a caller gets back exactly the gain the source WAV encodes" — meeting a fixture ~28 dB hotter than the ordinary ones and a chain whose only ceiling is FR-CHAIN-090's hard clamp. `PreparedIr` gains `normalize_gain_db()` (measured post-resample, post-truncation, over the taps the convolver actually runs); `namir-params` gains `ir.normalize_enabled` (Stepped, default on); the `Ir` stage applies it per slot through a `GainRamp`, seated settled on the slot's first block. The requirement's `Verify: U` is `stages/ir.rs`'s `normalisation_is_applied_and_defeatable`, which asserts both clauses end to end against `direct_convolve` and deliberately uses a *quiet* IR so a sign error cannot pass. **The xrun half of the same report is not fixed here and is not this change**: measured on a dev machine at the reporter's own settings (48 kHz, 480-frame blocks, standard WaveNet, gate + EQ active), the whole six-stage chain costs 4.9% of the block period with a 1024-tap IR and 7.2% with the 10-second one (p99 8.9%, max 12.3%) — a real but small cost curve, nowhere near a missed deadline, and the denormal hypothesis was tested and refuted in the same run (a silent decaying tail measured no slower than the driven one). The cause was already found and fixed on `trunk` before this change was written: `namir-app`'s duplex bridge ring was never primed, so its steady-state occupancy at pull time was zero and every output callback that ran ahead of its input partner padded and counted an xrun, perpetually rather than only at startup (`a8bd11b`, PR #163 — one block of prefill, at the cost of one block of latency the round-trip estimate already assumed). The report was made against a branch predating it. Recorded here because the two symptoms arrived in one report and the natural reading — a long IR is expensive, therefore it caused the xruns — is wrong, and the numbers above are what say so. **One deliberate choice in FR-CFG-020's shell-parity vector**, whose `PARAM_OVERRIDES` table must name every `REGISTRY` key and whose default answer for a new key is a non-default value: `ir.normalize_enabled` is entered **at its default (on)** instead, and the golden preset regenerated with it on. Off would replace a gain derived from the loaded IR's own `sum(h^2)` with a compile-time `0.0 dB` — exactly the setting under which a shell that derived it wrongly, or applied it on one side only, would still compare bit-identical. Off is the weaker test here, not the stronger one; `nam.normalize_enabled` sits in that table at its default for the same structural reason, and the table's "is at its default" assertion is scoped to `ParamKind::Continuous` for this class of case. That test runs behind `cargo test -p namir-clap --features host-ext-tests` (D-18.7), not `cargo test --workspace`, which is why the workspace gate was green while the table was short an entry. **The same hole, one gate over, is closed in the same pass**: the workspace clippy step walks default features too, so nothing had ever linted the code `host-ext-tests` compiles, and it had accumulated three `-D warnings` errors (`chunks_exact_to_as_chunks`) that no CI run could see. CI gains a `cargo clippy -p namir-clap --all-targets --features host-ext-tests` step in the same named, single-feature form the test step already uses — **never `--all-features`**, for R-17's reason — and the three sites are fixed. |
| 0.42 | 2026-09-07 | **Issue #166: the output stream stops forcing a period on the device (new Decision D-13.3).** Reported as an over-long IR causing crackling and endless FR-IO-060 xruns, and the natural reading — a long IR is expensive, so it missed its deadline — was wrong twice over. The chain never exceeded its period in any run (`over-budget = 0` throughout, a 950 µs callback against 10 ms), and the defect reproduces with **no IR and no model at all**, using a spin loop injected into an empty chain. What was actually happening: `audio_io::stream_config` requested `BufferSize::Fixed(negotiated)` for both streams, which for the output stream forces the device onto a period it did not choose, and the callback's duration then comes straight out of the output clock — measured at `period = nominal + callback duration` to three decimals. The output stream produced ~4.5% less audio than real time, the bridge ring filled, and `push_captured` began refusing captured frames: that is the crackle, and every xrun after the first. The fix is one line at two call sites behind `audio_io::output_buffer_request`, plus `estimate_round_trip` taking `Option` for the output term and printing `at least ~` when the device chose. **Two things recorded rather than smoothed over**: FR-IO-040's buffer-size selection no longer reaches the output device (D-13.3's Consequence books the gap; it still sizes the engine block and the input stream), and the mechanism behind the headroom difference is unexplained — the measurement is one interface in exclusive mode, and D-13.3 says so. Two earlier diagnoses in this same investigation were wrong and are not preserved as decisions: that PR #163's bridge prefill explained it (it had already landed, and the symptom outlived it), and that a *larger* requested buffer would restore slack (tested; scaling the request scales the callback with it, preserving the ratio exactly). `docs/manual-tests/fr-io-050-latency-measurement.md` gains an executed supplementary note; `docs/user-guide.md`'s sample line follows. |
| 0.43 | 2026-09-07 | **The first trainer-produced A2 exports are compared against `NeuralAmpModelerCore`; R-9's item (a) closes and two parser defects are fixed on the way.** Reported as a user's Tone3000 downloads failing with `nam.load.unsupported_architecture`, which turned out to be three separate barriers rather than one. Two were parser defects of the same shape: `reject_unsupported_layer_features` refused `gating_mode` and `secondary_activation` on the *presence* of the key, while the `UNSUPPORTED_CONFIGURATION` catalogue entry and the fields' own declarations had always described a by-value rule — and both fields are inert in every real export (all `"none"`, all `null`), so all 126 submodels of a 63-file sample were refused for naming features they had switched off (#169, #170; the reference's own shape detector agrees, `a2_fast.cpp:837-859`). The third barrier is `SlimmableContainer`, which every recent Tone3000 upload now uses and which D-9.12 defers by decision; left open as #172, with the reference's selection rule now recorded there (`container.cpp`: thresholds, defaulting to the last submodel). With the first two fixed, ten submodels from five real exports match the pinned reference at **-104.96 to -124.73 dB** and a real A1 control at -137 to -138 dB, recorded in the new `docs/manual-tests/fr-nam-030-real-a2-models.md` — closing **R-9 item (a)** and roadmap §21 Phase 4b's "no genuine A2 export has ever been loaded" bullet, both of which gain dated notes; AGENTS.md's testing-philosophy section loses the same clause. The measurement first reported *every* model failing at -29 to -58 dB, and a real-A1 control run is what showed the harness rather than A2 was at fault: `NeuralAmpModelerCore` prewarms (`NAM/dsp.cpp`'s `DSP::prewarm`) and `namir-nam` does not, so `xtask nam-parity` was comparing a settled reference against a cold namir and reporting the difference as an FR-NAM-030 failure. `nam-parity` now prewarms; the product gap it exposed — namir never prewarms a loaded model, so its first ~85 ms diverges by ~-30 dB on every load, including mid-performance switches through D-8.1's handover — is **#173**, unfixed. Two committed comments in `golden_reference.rs` claiming `WaveNet` does not override `GetPrewarmSamples` are corrected: it does, and the goldens' -132.58/-126.46 dB figures are unchanged because `namir-fixtures` emits every bias as exactly zero, making a prewarm a numerical no-op on those fixtures alone. Also opened: #171 (per-layer array lengths unvalidated), #175 (retargeting a PR starts no CI), #176 (comment prose over 100 columns in 157 places). |
| 0.44 | 2026-09-07 | **Issue #173: models are prewarmed on load; new Decision D-9.13 states that prewarm is part of matching the reference rather than a host convenience.** The reference runs every stateful model on a length of silence before its first real sample (`NAM/dsp.cpp`'s `DSP::prewarm`) and Namir did not, so a freshly loaded model spent its first ~85 ms at roughly -30 dB of error against a reference that agrees to -105 to -138 dB once settled — the whole of the discrepancy PR #174 measured on real exports, in full. `PreparedNam::prewarm_samples()` mirrors `GetPrewarmSamples()` per architecture (WaveNet's receptive field, LSTM's fixed half second) and `new_state_prewarmed()` applies it; `namir-engine`'s `NamSlot::new` calls it, which puts the work in **D-8.1's prepare step on a worker thread** — the audio thread pays nothing, and NFR-RT-010 is untouched. **Verified rather than assumed, in both directions.** The A2 arithmetic checks out independently: `prewarm_samples()` returns **6 347** for both real A2 shapes, which is the 6 346-sample receptive field this document and two test files already recorded, plus the `1` issue #173 quotes the reference as adding for an absent `condition_dsp` — a number derived from `Conv1D` fields agreeing with one written down from the C++ months earlier. That agreement corroborates the *layer* arithmetic and not the head term, since these shapes have exactly one array with a kernel-over-1 head and the implementation's deliberate over-estimate is therefore zero for them; the reference is not vendored here, so the formula's attribution rests on the issue's quotation and is labelled as such at the accessor. And the four committed goldens were moved onto the production prewarm path and still print **-137.06 dB** (WaveNet-A1), **-132.58** (A2-Full), **-126.46** (A2-Lite), LSTM still to the bit, because every bias `namir-fixtures` emits is exactly zero and a prewarm on such a fixture is a numerical no-op — so no committed figure goes stale, and the goldens now assert the path a host actually runs rather than a warm-up performed only by the test. `xtask nam-parity` drops the deliberate one-second over-estimate commit `bbe7d2a` introduced as a stopgap and asks the model for its own count instead, comparing from sample 0 with no offset arithmetic at all; it reproduces -132.58 dB from that second code path. (Not a change of compared *span*, and an earlier draft of this row said it was: `bbe7d2a` prepended a second of silence and discarded exactly what it had prepended, so all 480 000 real samples were already being compared. What changes is the prewarm length, not the span.) **Three committed claims this makes false are corrected in place rather than left standing**: `namir-nam`'s crate-level doc comment and `lstm.rs`'s "**Not ported:** `LSTM::GetPrewarmSamples`… a known, undocumented-elsewhere gap", both of which recorded prewarming as deliberately test-only. The wrapper-not-kernel fact they rested on is accurate; the conclusion drawn from it was not, since every host using the reference gets that wrapper. FR-NAM-030's text is **unchanged** — D-9.13 records that the requirement already required this, which is the disposition issue #173's scope note asked for and did not presume. New `crates/namir-nam/tests/prewarm.rs` pins the counts against the reference's own arithmetic, asserts bit-exactly that further silence past the prewarm length changes nothing (the property that makes WaveNet's over-estimate exact rather than approximate), and checks that a prewarm moves a non-zero-bias model at all, so the goldens' no-op cannot make the rest hold vacuously. **Review on PR #179 found that the settling assertion was itself vacuous on its first attempt, and it is worth recording because the failure is the same one this row congratulates itself for avoiding elsewhere**: it compared two freshly prewarmed states, and with every `namir-fixtures` bias exactly zero the settled state *is* the zero state a fresh `new_state` already starts from, so both sides were bit-equal for reasons unrelated to the receptive field — the assertion would have held had `prewarm_samples()` returned `0`. It was the cited evidence for the head-term over-estimate and for this decision's own clamping argument, and could carry neither. Rewritten to drive real signal through first, so the claim under test becomes whether a prewarm's worth of silence is enough to *flush* non-zero histories, which a wrong receptive field fails. Verified to have teeth rather than assumed: forcing `prewarm_samples()` to return 64 against the true 6 347 makes it fail. Two further corrections from the same review, both committed statements that were false: this row and the manual-test document each claimed `nam-parity` now compares "480 000 samples rather than 432 000", when `bbe7d2a` had discarded exactly the silence it prepended and was already comparing all 480 000 — what changed is the prewarm *length*, not the span; and `MAX_PREWARM_SAMPLES`' own headline still carried the `1 << 20` figures (1 048 576, ~21.8 s) after the constant was tightened to `1 << 18`, contradicting the ~5.5 s bound the rest of the same comment argues for. |
| 0.45 | 2026-09-07 | **Issue #172: `SlimmableContainer` models are supported (D-9.12 consequence note added).** (Renumbered from 0.44: PR #179, issue #173's model prewarm, claims 0.44 on trunk; 0.44 appears here when it merges.) `namir-nam` recognises `architecture: "SlimmableContainer"`, validates submodel `max_value` ordering/terminal threshold and sample rates, and prepares the last submodel through `model::load`. `probe_metadata` extracts container/submodel metadata without allocating weights. `condition_dsp`, FiLM conditioning and `.namb` remain deferred. **Second review round, and the finding worth recording is that the container work broke something that had nothing to do with containers**: giving `ProbeShape` a typed `config` field imposed a container's shape on *every* document, and `#[serde(default)]` covers an absent key rather than a present one of the wrong type — so `"config": null`, `5`, `[1,2]` and `"x"` became hard rejections of plain WaveNet and LSTM files a probe had always accepted, and in `namir-library` such an entry stays indexed while silently losing its name, architecture and sample rate. `null` is the shape AGENTS.md's testing-philosophy section names from a real post-M6 defect and that the generated fixtures structurally cannot emit, so nothing in the suite caught it. The container reading moves to a second, container-only parse performed only for a file whose first pass already said `SlimmableContainer`, restoring the module's "the first pass never looks inside `config`" invariant instead of deleting the paragraph that stated it. Three further gaps closed in the same round: `MAX_SUBMODELS` now bounds the **probe** as well as the load (the bounded path was rejecting over-ceiling containers while the unbounded one accepted them and allocated a `String` per metadata field per submodel — backwards, since a library scan walks every file through the probe and swallows the error, while a load runs only on a file the user chose); a container-wide sample rate declared by a *non-selected* submodel is now applied rather than merely cross-checked, closing the third instance of the same probe/load-disagreement family as review findings 1 and 2 and latent for the same reason (all 63 real Tone3000 containers declare the rate on both submodels); and an unreachable `!max_value.is_finite()` branch is deleted, its test having asserted `NON_FINITE_VALUE || MALFORMED_JSON` and passed on the second disjunct every time, verifying nothing while making the dead branch look covered. `merge_metadata`'s three copies — `wavenet.rs`, `lstm.rs`, and an inline third in `probe_metadata`, each enumerating all six `NamMetadata` fields by hand — collapse to one `NamMetadata::fill_empty_from`, so a seventh field cannot be silently dropped by all three. **The round's most consequential finding is the one that changed behaviour rather than structure**: asked whether the container invariants reject files the reference accepts, the answer was obtained by *reading the pinned reference* — checked out on the machine these parity runs already use — instead of restating the citation. Two of the three invariants are verbatim reference behaviour; the sample-rate one was stricter, because the reference compares each submodel against the container's own rate and skips the check when either side is unknown, never comparing submodels to each other. A container declaring no top-level rate whose submodels differ was refused here and loads there. Corrected, with the pre-existing test that had pinned the stricter rule rewritten rather than deleted, and the tolerated case pinned in its place. **The same reading withdrew a fix requested in the same review**: item 10 asked that a rate declared only by a *non-selected* submodel be propagated to the loaded model; the reference consults only the active submodel's own rate when the container declares none, so propagating would have diverged in the opposite direction, and the probe/load asymmetry the item observed (`None` versus 48 000) is the documented "absent means the 48 kHz convention applies" contract rather than a disagreement. Recorded because a review comment declined on evidence is worth as much on the record as one acted on. See D-9.12's second consequence note for the citations. |
| 0.46 | 2026-09-07 | **Issue #94: a preset recalled from the plugin's own editor now wakes the host main thread, so the host re-reads its stale parameter cache.** `spawn_recall_preset` runs on a worker-pool thread and could only park `params_rescan_pending` — the `HostParams::rescan` that services it is `[main-thread]`, and nothing was guaranteed to schedule a callback, so the host kept stale values until a latency or thread-priority event happened to wake it. The fix stores an erased-lifetime `HostSharedHandle<'static>` in a designated unsafe module `crates/namir-clap/src/host_wake.rs` (D-5.3's written safety argument: the `clap_host` pointer outlives the instance until `destroy`, `NamirShared::drop` joins every worker thread before that, `ThreadPool::spawn` drops post-shutdown jobs, and `request_callback` is explicitly thread-safe; teardown re-entrancy hazard documented) and has the recall job call `request_host_callback` after setting the pending flag. D-5.3 gains its *Consequence (added M15, 2026-09-07, issue #94)* note; the workspace's designated-unsafe surface moves from three files to four (D-5.3 and AGENTS.md both updated). Review on PR #181 surfaced three test-seam findings addressed in the same pass: `host_wake`'s unit test counter moved inside `if let Some` and `worker_jobs`'s test backed by a static `clap_host` with teeth (deleting `handle.request_callback()` fails); `__test_support` holds `Weak<SharedInner>` cleared on `destroy` so instances never leak; and `CURRENT_THREAD_SHARED` thread-local isolation prevents parallel libtest instances from colliding. |
| 0.47 | 2026-09-07 | **Issue #23: the signing decision is taken and the CI half is built; R-11 narrows to the credentials.** The owner has decided to obtain an **Apple Developer ID** for 1.0, and `release.yml`'s macOS job gains the step the M13 restatement at R-11 said was unbuilt: `import_cert` imports a base64-encoded `.p12` into a temporary keychain, gated on job-level `HAS_SIGNING_CERT` (handling GitHub Actions' limitation where the `secrets` context is unavailable in `if:`), using `security create-keychain`/`unlock-keychain`/`import`/`set-key-partition-list` (`-T`-limited to `codesign` and `productbuild`), an exit `trap 'rm -f ...'` ensuring decrypted `.p12` cleanup on any failure path, and an `always()` cleanup step deleting the keychain whenever `steps.import_cert.outcome != 'skipped'`. `make_installer.sh` reports whether it signed and notarized via `$GITHUB_OUTPUT`, exposed as macOS job outputs (`signed`, `notarized`). The `publish` job reads those job outputs directly rather than re-deriving from secrets: when signed and notarized, the release notes describe the macOS `.pkg` installers as signed, notarized and stapled, give valid verification commands (`spctl --assess --type install -vv` and `xcrun stapler validate`), explain that the `.pkg` inside `.dmg` is the recommended path because files extracted from the `.zip` (though signed with Developer ID Application via `ditto`) lack stapling and carry the quarantine attribute requiring an online check, and unconditionally report the Windows SmartScreen status; otherwise, the notes keep the unsigned macOS wording while still reporting Windows SmartScreen status. `packaging/macos/README.md` documents the seven secrets and the `.p12` export/encode commands (`base64 -i Certificates.p12 \| tr -d '\n'`). What is recorded as still open is the credentials side only: the purchased identity, the secrets configured in the repository, and a first signed tag run — the pipeline is otherwise built and ready. R-11's row and the roadmap's Phase 6 bullet are updated to say so; the risk stays **Medium** until a signed run exists. |
| 0.48 | 2026-09-08 | **Issue #167: requested buffer size is preserved across fallbacks and clean install leaves it absent (D-13.1 consequence note added).** Hand-edited `buffer_size_frames` in `audio-settings.json` was previously overwritten by fallback negotiation whenever a device declined the requested buffer size. `persist_negotiated_audio` now preserves requested buffer size rather than pinning fallback values, leaving `buffer_size_frames` absent (`None`) on a clean install; `app.audio_io.buffer_size_declined` warns non-modally when fallback negotiation differs from the requested size. `AppSettings` and `ChannelMapping` adopt `#[serde(default)]` to cleanly deserialize partial configurations, trading corrupt-file detection for truncated valid JSON. |
| 0.49 | 2026-09-08 | **Four `Verify: M` UI Musts are executed on real hardware, and the Escape key stops committing the edit it was meant to cancel.** (Numbered 0.49 because PR #183's issue-#167 row landed first and holds 0.48 — the collision 0.45's own note records, resolved the same way.) FR-UI-020, -040, -050 and -070 were run by a human on §2's reference machine in both product configurations, taking the uncovered-Must count from 8 to 4 and §14's `5.13 UI` row from 3 / 4 / 0 to 6 / 1 / 0 (Total 69 / 61 / 0) — the movement, its per-cell evidence and what it does *not* claim are in `docs/03-implementation-roadmap.md`'s dated subsection, and §22 **R-16**'s "still read NOT EXECUTED" clause gains a note saying the runs happened outside CI, which is what that row is about. **FR-UI-040's step 6 failed first, and the defect is a two-frame window in `egui` 0.35**: `DragValue::update_while_editing` defaults to true, so every keystroke dispatched a `SetParam`; and `Memory::lost_focus(id)` answers `id_previous_frame || id_two_frames_ago`, so on the frame after Escape the focus-loss path parsed the staged text and committed the cancelled edit. The fix is three lines in `crates/namir-ui/src/controls.rs`, each load-bearing — turn off live update, drop the staged `String` when the widget loses focus to Escape, and swallow the one no-op `changed()` the first line introduces — pinned by a red/green multi-frame script in `crates/namir-ui/tests/ui_interaction_scripts.rs` and explained in a comment at the site, since deleting any one of them silently restores the bug. The four manual documents' prior records are **kept and appended to** rather than rewritten: superseded verdict lines keep their text with the `**Result:` marker demoted, so the gate reads one live verdict instead of the worst of several. |
| 0.50 | 2026-09-08 | **The audio-device panel is built (PR #159, issue #26 closed); roadmap §15 item 16 is answered by construction.** `UiSnapshot` carries `audio_panel_open: bool` and `audio_panel: Option<AudioDevicePanelSnapshot>` (`crates/namir-ui/src/host.rs`), and four new intents (`SelectInputDevice`, `SelectOutputDevice`, `SelectSampleRate`, `SelectBufferSize`) call `initiate_audio_reopen()` in `crates/namir-app/src/host.rs:1486-1513`, enabling dynamic in-session stream reopening. Decision D-15.3 already records the overlay panel consequence (added M15, 2026-09-06); no architecture decision claimed in-session selection was impossible. The panel carries no latency or xrun field, so FR-IO-050 and FR-IO-060 remain unclosed. |
...
| 0.58 | 2026-09-09 | **Recorded retrospectively (issue #199), closing a gap PR #187 left in this log: `namir_engine::ParamId` is gone, and the duplicate newtype's removal was never written down here.** Numbered 0.58 because that is the next unused number while the row sits at its own date, between 0.52 and 0.53 — the same numbering-versus-order split 0.49's note records, and the same retrospective correction 0.12 was written to make. `namir-engine` had declared its own `ParamId` newtype beside `namir_params::ParamId`, so every id crossing the boundary was converted; #187 (`1cca271`, merged 2026-09-09) deleted the engine-side copy and let the crate use `namir_params`'s directly. **D-10.2** gained its `*Consequence (added M15, 2026-09-09)*` note in place at the time, per this document's convention; the 30 `const *_ID: ParamId = X.id;` aliases in `chain.rs` and `stages/{trim,gate,nam,ir,eq,out}.rs` survive the deletion as short local names for each `apply`'s comparison chain — no `match` arm anywhere depends on them being consts — and each block now says so in one line rather than carrying the deleted two-`ParamId` rationale (issue #199). No behaviour change. |
...
| 0.54 | 2026-09-10 | **`import_cert` accepts a second, optional `.p12`, so two Developer ID certificates issued against separate CSRs can both reach the runner's keychain (issue #23 follow-up).** `NAMIR_APPLE_INSTALLER_P12_BASE64` and `NAMIR_APPLE_INSTALLER_PASSWORD` (the latter defaulting to the application `.p12`'s password) are imported into the *same* temporary keychain when set and skipped when not, so the existing one-file path is untouched; `set-key-partition-list` is per-keychain and covers both without repetition, and the `EXIT` trap is armed before the second decode so an abort between the two still scrubs the first. **The reason this is a workflow change rather than a documentation one**: `openssl pkcs12 -export` carries exactly one private key and discards any others in silence — verified, two keypairs and two certificates in, two certificates and one key out, no warning, exit 0 — so a contributor without Keychain Access cannot express two keypairs as one file, and the previous single-secret design silently forced one CSR to be submitted to both requests. Shaping the certificate setup around a limitation of the packaging tool was the wrong way round. `packaging/macos/README.md` drops its one-CSR rule for "one CSR or two, but know which you did", documents both `openssl` shapes, and now leads its encode step with `openssl base64 -A` (one command, one line) instead of `base64 | tr -d '\n'` — without `-A` a 2 KB `.p12` wraps to 74 lines, which is exactly the corruption the surrounding paragraph warns about. |
| 0.55 | 2026-09-10 | **The reverse-DNS identifiers move from `org.legrand` to `io.namir`, on acquisition of the `namir.io` domain.** The plugin's CLAP id and `CFBundleIdentifier` become `io.namir.clap` (one string in two places, `crates/namir-clap/src/lib.rs`'s `PLUGIN_ID` and `xtask/src/bundle.rs`'s `PLUGIN_BUNDLE_IDENTIFIER`, still required to match); the standalone's becomes `io.namir.standalone`; the three `installer` component ids follow, with `PKG_ID_APP` renamed `io.namir.standalone.pkg` so the package tracks its bundle's spelling. **Timing is the substance of this entry, not the strings.** `PLUGIN_ID` is what a CLAP host keys a saved session to, so changing it after a release silently orphans every project that had loaded the plugin; `git tag -l` was empty when this was made, so the change costs a commit rather than the users' projects, and it cannot be made this cheaply again. The justification is that a reverse-DNS identifier should name a domain the project controls, which `org.legrand` never did. M13's decision at `APP_BUNDLE_IDENTIFIER` is **not** rewritten: it gains a `*Consequence (added M15, 2026-09-10)*` note separating what survives (unique-per-bundle, TCC keys the microphone grant on the app's id, `.app` still rejected as reading like the bundle extension — hence `.standalone`) from the one clause the domain supersedes (the plugin no longer has to *be* the product root, since `io.namir` carries the product name, so `clap` and `standalone` are siblings and a later `io.namir.vst3` is a peer of the first format rather than a child of it). Windows' Inno `AppId` GUID is deliberately untouched — its own comment says it must never change, and only its cross-reference to the macOS spelling moved. |
| 0.56 | 2026-09-10 | **A2's per-layer gating arrays are now length-validated against the layer count (issue #171).** `gating_mode` and `secondary_activation` are one entry per layer, and `reject_unsupported_layer_features` read only whether every entry was inert, never how many there were — so a `["none"]` against 23 layers loaded here and was refused by `NeuralAmpModelerCore`. The check goes to `resolve_layer_array` under `INCONSISTENT_CONFIGURATION`, alongside the `kernel_sizes`/`activation` length checks, **not** to the inertness checks under `UNSUPPORTED_CONFIGURATION`: a wrong-length per-layer array names no unimplemented feature, it disagrees with the file's own `dilations`, and attaching "this build does not support that" to a self-contradictory file is the false-message failure FR-NAM-140's distinct-code requirement exists to prevent. Three limits, each the reference's own: the array form only (`gating_mode: "none"` is a length-less scalar with its own path at `model.cpp:1068-1082`); `secondary_activation`'s length read only when `gating_mode` is an array (`model.cpp:1049-1062`, so a short `secondary_activation` alone still loads, as it does there and did here); and the feature check still running first, so an *active* wrong-length array reports the gating rather than the length. **One deliberate reversal:** the empty array `[]` no longer loads. It is a wrong-length array, not an absent field, `size() != kNumLayers` has no exemption for it, and `an_inert_gating_mode_is_accepted_however_it_is_spelled` — which asserted the old behaviour — was updated rather than worked around. No real file is affected: all 126 submodels surveyed in issue #171 write correct-length arrays. The 126-submodel probe was not re-run for this change (the files are not committed, per D-19.1). |
| 0.57 | 2026-09-10 | **Stopping a stream no longer counts an xrun of its own (issue #194).** `RunningStreams` dropped its two stream fields in declaration order, so the input side stopped while the output side was still being serviced by the driver; every output callback in that window pulled a bridge nobody was feeding, `BridgeConsumer::pull_into` padded, and `build_output` charged FR-IO-060's session counter for a dropout the shutdown itself invented — at every quit and every audio-settings change. An explicit `Drop` now stops the output side first (`crates/namir-app/src/stream.rs`), with the fields held as `Option` purely so that order is written down rather than inherited from field declaration order, which a later reorder could silently invert. `pause()` follows the same order; `play()` keeps the mirror image, input first, because the bridge must already be filling before the output callback pulls it. **One correction to the issue's own cause section, which is the part worth recording:** it claims the reopen path overlaps two live pairs at `hold_streams`. It does not — `initiate_audio_reopen` already drops the old pair before requesting the engine rebuild (D-15.3, D-8.1: the old callback must stop before the instance is replaced), so nothing overlapped and no silent-window trade had to be made. The residual is a margin, not a proof: the capture side's undrained ring holds ~7 blocks past the prefill, ~75 ms at a 480-frame block but ~4.7 ms at the 32-frame minimum, and an overrun there records on the same counter — better than the old order by seven blocks of slack rather than one at every block size — the old order had only the one-block prefill — and worse only if closing the output side takes some seven blocks longer than closing the input side, which no observed teardown does. Not a guarantee at the small end. Issue #189's startup activation transient is untouched and still contributes its two pads per open. |
| 0.59 | 2026-09-11 | **Issue #200's document half: R-10 is restored and appended to, and §17 stops giving two answers for the `cpal` fork.** PR #188 rewrote R-10 in place, which deleted the M11 upstreaming analysis (upstream `cpal` issues #1220, #843 — with the warning not to cite #843 as upstream refusal, since its author closed it himself and no maintainer reviewed it — and #1195; #28 depends on that research), the "2867 insertions / 144 deletions across 11 files" measurement, and the Mitigation column's "never by branch" clause plus its M11 note about `license-audit` gaining `cargo deny check sources`. All of it is back verbatim, with #188's note reproduced beneath it and a correction appended: `baseview` is **not** pinned by a manifest `rev` — `Cargo.lock:163` resolves it as `git+https://github.com/ErwanLegrand/baseview.git?branch=accesskit-design#c0870cb…`, a branch spec arriving through the `egui-baseview` fork's manifest, so `cargo update` can move it. The remedy is one line in that fork's own manifest and is not expressible here (a `[patch]` cannot point at the same source; a direct `rev` edge conflicts with the transitive branch spec), so it is an open action on the fork owned by issue #200. §17's `baseview` row now says the `c0870cb` is a lockfile pin; the detailed M11 `cpal` fork row keeps its text and gains a **Superseded 2026-09-09 (PR #188)** note naming the four facts #188 falsified; and the head fork row's Risk is reverted `Low` → **High**, since nothing made the fork less risky and #188's 0.18.1 → 0.19.0 rebase is the event R-10 warns about. The 11-files and seven-files figures are both kept as dated measurements against different bases (`e0893c3` at M11, `381cf1d` at #188) rather than one being picked. No manifest and no `deny.toml` change (its comment was already correct). **Amended after review (PR #205):** item 1's contract now holds for *every* caller — `crates/namir-ui/examples/manual_window_smoke.rs` called both `EguiWindow::create` and `run_until_closed()` inside the `open_with_srgb_fallback` closure, the shape the new doc comment forbids, and it is the caller the required `headless window smoke (FR-UI-010, issue #143)` job runs. There a panic in `namir_ui::render` at frame 30 unwound into the `catch_unwind`, the retry re-zeroed the frame counter and drew 90 clean frames with `srgb = false`, and the job went green on a broken interface; the closure now returns the window and `main` drives the loop after it. The stale prose describing `open_blocking`'s pre-#200 run-on-its-own-thread-and-join behaviour is corrected in the same breath: the example's header rationale, its frames-short failure message and `.github/workflows/ci.yml`'s step comment. **Added after review (PR #205):** the Escape-cancellation stamp's unit was questioned — `cumulative_pass_nr` counts passes, and a `request_discard` re-run could consume the stamp inside the same frame. Settled by running it rather than by reading: on a two-pass Escape frame, pass N has `lost_focus && Escape` and stamps; pass N+1 of the *same* frame carries no events at all (`Context::run_dyn` takes the `RawInput` once), so the stamp is not rewritten — and does not need to be, because `DragValue`'s own focus-loss re-commit lands on that same pass. "Pass" is therefore the correct unit on both sides; a frame-scoped stamp misses the re-commit, which is what `escape_cancellation_survives_a_multi_pass_frame` fails on. A second review round asked for a test proving a stamp older than one pass no longer suppresses; instrumenting `param_control` showed there is no such case to assert in egui 0.36. The old `bool` was consumed on the first pass the control rendered again, whatever pass that was, and two mechanisms stop a legitimate edit landing on that pass: `DragValue`'s focus-loss re-commit window (`Memory::lost_focus`) is spent while the widget is absent, and egui derives interaction from the previous pass's rects, so a click delivered on the return pass is dropped. The pass-number stamp is therefore a defensive tightening with no reachable behavioural difference, not a bug fix, which is why no stale-stamp regression test accompanies it. |
| 0.60 | 2026-09-11 | **A stream's activation transient is no longer counted as a dropout (issue #189).** Every healthy session opened at `xrun count is now 2` before the user played anything, and every buffer-size or device change added two more. Two causes, both measured on §2's interface rather than reasoned about: the output callback fires once and the device then takes ~524 ms to actually start running (one callback, the gap, then an exact 10.000 ms cadence for the rest of the run), and — after both sides are running — WASAPI shared asks **480 frames per output callback against 256-frame captured blocks**, so the pull, chunked at 256 + 224, starves once more before the ring reaches steady-state occupancy. `open`'s one-block prefill covered only the first pull. `open` now owns a `capture_started` latch, set by the input callback, and `build_output` counts a pad only past a bounded settling window of pulls anchored on that latch — so an activation gap of any realistic length is covered, rather than a fixed number of callbacks from the open. **Both halves are bounded, and the second bound came out of this PR's own review:** waiting on the latch alone meant a capture side that opens and then silently delivers nothing — which raises no `StreamFailure`, there being no error for the driver to report — suppressed every pad for the life of the stream, so the one failure FR-IO-060 matters most for read a clean 0 (demonstrated in review at 200k callbacks, ~3.5 h of audio). A second ceiling restores the signal without weakening the transient fix. **Both budgets are wall-clock rather than pull counts, corrected in this PR's own review:** a pull is at most `max_block` frames and `STANDARD_BUFFER_SIZES` offers 32..=2048, so a fixed pull count swings 64x in real time — a 512-pull ceiling is ~2.7 s at 256 frames but only ~341 ms at 32, *below* the measured ~524 ms activation it exists to outlast, which would have brought issue #189 back at exactly the buffer sizes a latency-sensitive user picks. The ceiling is therefore 3 s and the settling window 100 ms, each divided into pulls once per open in `build_output` before the closure is built (the RT path keeps two counters, one relaxed load and one comparison, and no division). **The trade is stated rather than hidden:** a genuine dropout inside the post-latch window (100 ms, at or above the ~85 ms the 256-frame reference machine was measured with) is not counted; the alternative is every session and every settings change opening at a non-zero count, which is exactly what a user reads as "my audio glitched". Both pieces of state are local to the closures `open` builds, so the suppression is per-open by construction — a session-scoped flag would have fixed launch and left every reopen counting two. Verified live: three idle 12–15 s sessions on §2's machine reach the log's end with no `xrun count` line at all, where the same build before the change printed `2`. Numbered 0.60 while the row sits at its own date; 0.58 and 0.59 belong to the two document PRs open alongside it. Related but separate: 0.57 (issue #194) removed the *teardown* pads, and issue #200 item 6 owns the opposite gap — `cpal` 0.19's `CallbackInfo::xrun()` reaching no counter at all. |
| 0.61 | 2026-09-11 | **Start-up and reopen assemble one audio configuration through one function (issue #192).** PR #191 shared the enumerate → negotiate → re-enumerate-on-downgrade chain; everything *between* the negotiation and `crate::stream::open` was still built twice, in `app::run` and `AppHost::initiate_audio_reopen`, from the same values in the same order — both `StreamParams`, D-13.3's `output_buffer_request()` line, `block_frames`, the `ChannelConfig` choice, the validated `SampleRate`, and the two supported lists FR-IO-040's selectors show. `app::assemble_stream_config` now returns all of them as an `AssembledAudioConfig`, and each caller keeps only its own failure handling: start-up prints to stderr and opens without audio, the reopen path posts a notice and leaves the running stream up. That asymmetry is why this is not folded into `negotiate_audio`. The invalid-rate case is carried as `sample_rate: Option<SampleRate>` rather than decided inside the shared function, for the same reason. **Why it mattered:** a fix applied to one copy is silent, because the reopen path runs only on a settings-panel change — which had already happened twice, with D-13.3's line (issue #166) and #190's re-enumeration both landing in one copy first. `host::tests::a_reopen_assembles_its_stream_config_through_the_shared_function` drives the real reopen path (`dispatch` → `initiate_audio_reopen`) against a `FakeBackend` and asserts the `pending_reopen` it stores equals `assemble_stream_config`'s output for the same negotiation, so a re-inlined copy here fails it; `assemble_stream_config_produces_the_values_a_stream_opens_with` pins the derived values themselves, and dropping the `output_buffer_request()` line fails both (`Some(256)` vs `None`). No behaviour change intended, and none observed in `namir-app`'s suite. Numbered 0.61 while the row sits at its own date; 0.58–0.60 belong to the PRs open alongside it. |
| 0.62 | 2026-09-11 | **Backend-detected dropouts are counted again, through a widened `AudioBackend` seam (issue #200 item 6).** `cpal` 0.19 moved xrun delivery off the error callback onto `CallbackInfo::xrun()`, and this crate's stream-callback signature discarded it (`move \|data, _info\| on_data(data)`), so device-side dropouts were counted **nowhere** and `crate::bridge`'s ring under/overrun detector was FR-IO-060's only live source. `AudioBackend`'s two data callbacks now carry a Namir-owned `audio_io::CallbackStatus` — D-13.1 holds, no `cpal` type crosses the trait, and the fake backend still needs no hardware — whose `xrun` field `crate::stream` records into the same `XrunCounter` the bridge detector increments, so the two sources stay commensurable. `StreamFailure::Xrun`, which had had no constructor anywhere in the tree since the 0.19 adoption and whose `to_stream_failure` test arm had already been lost, is **deleted** rather than retained for a seam that has now been extended; `app::stream_failure_sink` lost its counting branch and its `XrunCounter` parameter with it, and `host::stream_failure_code` its arm. `CallbackStatus` also carries `first_of_callback`, a device-callback boundary the integer-converting path (`audio_io::convert`, where exclusive mode's formats are handled) sets on its first scratch-length slice only. That exists because both sources fire for one physical dropout and the bridge records were per chunk: with a `recorded` latch reset on that flag, **one device callback contributes at most one xrun on every path**, which is the over-count axis issues #189 and #194 were filed about. The struct is `#[non_exhaustive]` so a future backend cannot spell a partial literal and inherit a stale latch, whose failure mode would be silent under-counting. The counter's RT contract is unchanged: one relaxed `fetch_add`, no allocation, no lock. Not verified against hardware: whether a real driver raises `CallbackInfo::xrun()` at all, and what it does around stream start and stop — `docs/manual-tests/fr-io-060-xrun-induction.md` stays **PARTIAL** and records the unconditional-counting choice made in that absence. |
| 0.63 | 2026-09-11 | **How `baseview` could be pinned, re-run rather than restated (issue #200, item 2).** Three claims in 0.59's parenthesis were reproduced against the tree. A `[patch]` at the fork's own URL is refused — `cargo generate-lockfile` aborts with ``error: patch for `baseview` points to the same source, but patches must point to different sources`` and writes no lockfile, which 0.59 has right. A direct `rev`-pinned edge at that URL does **not** conflict, and 0.59's "conflicts with the transitive branch spec" is **superseded**: resolution succeeds, a lockfile is written, and cargo silently adds a *second* `baseview v0.3.3` while `egui-baseview` keeps the branch copy — a failure mode strictly worse than a conflict, because nothing announces it. And a patch at a *different* URL is accepted, so "not expressible here" is too strong: a mirror of the fork (`{ git = "<mirror>", rev = "c0870cb5…" }`) resolves, substitutes, and leaves `baseview` a git dependency pinned by rev. It costs a fourth exact URL in `deny.toml`'s `allow-git` — without one `cargo deny check sources` exits 8 on the mirror — and a second repository to keep in step, which is why it is recorded as available-but-unattractive rather than taken. R-10 and `deny.toml` carry the detail; `Cargo.toml` and `Cargo.lock` are unchanged. |
