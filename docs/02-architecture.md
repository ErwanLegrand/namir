...
| 0.39 | 2026-09-06 | **AArch64 NEON vectorization verified and documented (issue #148).** Verified that `wide::f32x8` on `target_arch = "aarch64"` maps to pairs of 128-bit NEON intrinsics (`f32x4` / `float32x4_t`) rather than scalar fallback; documented the two-op 128-bit composition vs x86-64-v3 256-bit AVX at Risk R-4. |
