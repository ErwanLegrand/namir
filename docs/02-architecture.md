...
S-1 is the largest and gates the most numbers. S-2 can run last.
...
| OQ | Status | Where |
|---|---|---|
| OQ-1 — Rust vs C++ inference | **Pending — S-1** | §9.1, §19 |
| OQ-2 — real numbers for placeholders | **Pending — S-1, S-2** | §2, §19 |
...
| R-4 | NAM inference in Rust misses the accuracy or performance bar. | Medium | S-1 exists precisely to find this out cheaply; C++ binding is the costed fallback, and it is MIT. |
