...
| R-7 | ~~Crossfade doubles NAM cost transiently, eating the NFR-PERF-010 budget.~~ **MEASURED at M4, 2026-08-06 — this risk's own mitigation ("the benchmark measures the crossfade") is now discharged, and the result is that the risk as worded is half right, with the wrong half named.** `namir-engine/benches/handover_crossfade.rs` drives real handovers through the real command ring at NFR-PERF-010's own literal condition, at four swap rates, in four arms. Measured on the §2 reference machine under D-2.4's conditions (core 4, machine verified quiet, **six retained repetitions of ten — four discarded on D-2.4's own estimator check**): steady state **16.25-16.51%** (estimator 14.60-15.29%, agreeing to 1.2-1.9 points, and reproducing M3's certified NFR-PERF-010 figure); **a NAM handover alone stays within the 25% budget at every swap rate — worst 24.31%** — including a 94%-duty rate faster than any human audition, so the specific claim this row makes does not hold; an IR handover alone also stays within budget, worst **24.63%**, though with little margin; but **a NAM and an IR handover in flight simultaneously exceeds the budget at every rate tested, 25.06-31.49%**, reproducible to under 0.6 points and therefore not a measurement artifact. **The risk is real but its cause is not the one named:** it is not the NAM crossfade's 2x, it is two stages crossfading at once. | Medium (open, now quantified rather than speculative) | The cheap and targeted mitigation is a **worker-side scheduling rule: never allow a NAM and an IR handover to be in flight simultaneously** — hold one until the other's retirement arrives on the return ring, which the worker already drains. That eliminates the only over-budget arm by construction, costs one state bit per instance, and is squarely `namir-worker`'s decision. Deliberately **not** built at M4 (the operator chose to certify the measurement first rather than fix and measure together), so it is named here as the recommended next step rather than assumed. Note that shortening the crossfade toward FR-NAM-070's 5 ms floor is **not** a fix: it reduces the transient's duty cycle, not its peak, which is 2x regardless of duration. |
