...
| `cpal` (Namir fork) | 0.19.0 (`381cf1d`) | Apache-2.0 | 2026-09-08 | Standalone audio I/O; WASAPI exclusive mode (FR-IO-020). PR #188 rebased onto 0.19.0 — the exact event R-10 warns "can silently regress the Windows audio path" | **High** — a maintained fork, rebased onto a new upstream line. Nothing lowered the risk from M11's assessment; see the superseded row below for the original adoption record. |
...
| R-10 | **New, from M8-planning, 2026-08-08.** D-13.4's forked `cpal` is a maintained fork: it must be rebased as upstream moves, and every rebase is work that produces no user-visible benefit and can silently regress the Windows audio path. It is also the first non-registry dependency in the tree, which weakens the reproducibility NFR-SEC-040 (Should) wants and needs a named `cargo-deny` `[sources]` allowance that the default policy would otherwise reject. **Status note (added M11, 2026-08-11, from building it) — the rebase burden is higher than D-13.4 assumed, and the upstreaming exit is in better shape than this row's own wording implies.** *Burden.* The fork as pinned (`rev 2edbacb`) is seven commits on upstream trunk `e0893c3`: **2867 insertions / 144 deletions** across 11 files (`cpal-src/src/host/wasapi/` comprises seven of them). Rebased on 0.18.1 trunk, not on an unreleased branch. *Upstreaming exit.* cpal issue [#1220](https://github.com/RustAudio/cpal/issues/1220) lists shared-mode extensions under extension traits; PR [#843](https://github.com/RustAudio/cpal/pull/843) was self-closed and never reviewed, and must not be cited as evidence upstream refused the feature. Issue [#1195](https://github.com/RustAudio/cpal/issues/1195) tracks WASAPI endpoint enumeration changes. The open issue tracking this exit is #28. **Status note (added 2026-09-13, issue #35 — corrected from the 2026-09-09 version):** Risk now extends to three Namir-maintained forks (`cpal`, `baseview`, `egui-baseview`). `cpal` and `egui-baseview` are pinned by commit in manifests. `baseview` is now pinned by rev (`c0870cb...`), resolved by rev-pinning the `egui-baseview` manifest (672ea81, #214 closed). An `xtask git-sources` subcommand (added M15) now enforces the `?rev=` requirement at the lockfile level and will fail the build when a branch-pinned source is present. | Medium | Pin by commit hash in manifests, **never by branch**. Keep each fork's diff minimal. Offer the change upstream so the fork has a defined end (`license-audit` CI step now includes `cargo deny check sources`, added at M11). Vendor if the fork stops being maintainable. |
...
| 0.62 | 2026-09-13 | **M15 review sweep: R-10's branch-pin status resolved, dependency register corrected, git-sources gate inserted.** R-10's 2026-09-13 status note corrected: `baseview` is now pinned by rev (`c0870cb`, through `egui-baseview`'s rev-pinned manifest), closing #214 within this same PR. §17's `egui-baseview` row updated from 6752731 to 4a46c0a. The `xtask git-sources` subcommand is added to CI and README, completing the R-10 gate. Also: Escape-suppression flag renamed from pass counter to frame counter with frame-expiry semantics, fixing a multi-pass-layout correctness bug in FR-UI-040's late-commit guard. |
