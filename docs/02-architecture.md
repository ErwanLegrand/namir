...
| `baseview` (Namir fork) | 0.3.3 (`c0870cb`) | MIT OR Apache-2.0 | 2026-09-08 | Windowing; AccessKit integration | High — fork of low-adoption crate. **Pin correction (added 2026-09-11, issue #200): the `c0870cb` above is a `Cargo.lock` pin, not a manifest `rev` pin.** This fork is a *transitive* dependency arriving through the `egui-baseview` fork's own manifest as `branch = "accesskit-design"`; `Cargo.lock` resolves it as `source = "git+https://github.com/ErwanLegrand/baseview.git?branch=accesskit-design#c0870cb5b5279e4fff174ea8b407f19671e4fd5e"` (`Cargo.lock:163`), so `cargo update` can move it while the two manifest-pinned forks stay put. `deny.toml`'s `allow-git` comment records the same thing. R-10 carries the remedy (a `rev` pin in the `egui-baseview` fork's manifest, an open action on that fork) and why this repository cannot express it — narrowed 2026-09-11 (issue #200): no route at the *same* git URL expresses it; a mirror URL or vendoring does, at a cost R-10 states |
| `egui-baseview` (Namir fork) | 0.7.0 (`6752731`) | MIT OR Apache-2.0 | 2026-09-08 | egui windowing embedding | High — fork of low-adoption crate |
...
`docs/03-test-plan.md` on the strength of a script that does not exercise what the panel added —
