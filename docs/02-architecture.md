...
**Decision D-9.14 (2026-09-09; PR #162; amends FR-CHAIN-050 for one opt-in mode)** — A single
parameter, `global.independent_channels` ("Linked" / "Independent", default **Linked**), switches
Gate, Trim, Nam and Ir from the mono core to **one full instance per captured channel**. Every
piece of per-channel state is built in `StagePrep::prepare` / `NamSlot::new` / `IrSlot::new` — off
the audio thread — so the switch itself is a scalar assignment in `Stage::apply` and allocates
nothing. The number of independent channels is `ChannelConfig::input_channels()`, so the mode is
reachable in `Stereo` only.

*Rationale:* FR-CHAIN-050's Rationale is an argument about one source widened for stereo, and it is
correct about that case. It does not reach a `Stereo` input carrying two different performances — a
DAW parent bus summing two panned double-tracked takes — where Trim's −6 dB sum is not a widening
but a destruction of one take. The mode is off by default because the requirement's other clause
("double the cost") is true; see the cost consequence below.

*Rationale (why `input_channels()`, not `output_channels()`):* `MonoToStereo` reports two output
channels fed by one captured channel. Gating that predicate on the output count would run two
inferences over two bit-identical signals — paying FR-CHAIN-050's doubled cost to obtain exactly
its "no benefit" — and would do so in the standalone, which is the product that cannot reach the
control at all. Encoding the predicate in the engine rather than in the UI is what makes the
setting *inert* rather than merely hidden there, and answers the preset-portability question
directly: a `.namirpreset` written by the plugin with "Independent" on round-trips unchanged
through the standalone (FR-STATE-030) and changes nothing about how it sounds, because no
per-channel state exists for it to select.

*Consequence — the mode costs ~2x the shipped chain and exceeds NFR-PERF-010's budget on both of
the bench's statistics.* **Certified figures**, measured with
`crates/namir-engine/benches/six_stage_chain.rs` (`NAMIR_CHAIN_INDEPENDENT=1`) on §2's reference
machine, **quiet**, 5 repetitions per mode, **all ten passing D-2.4's validity check** — so unlike
the first attempt below, D-2.2's own raw `p99.9` is quotable here and not only the
contamination-immune estimator:

| statistic | Linked (shipped) | Independent | ratio |
|---|---|---|---|
| per-residue-minimum estimator | 14.40% of one core | **28.35-29.07%** | 1.97-2.02x |
| raw `p99.9` | 16.76% | **31.39-31.82%** | 1.87-1.90x |
| `p99` | 14.83% | 29.25-29.96% | 1.97-2.02x |
| `p50` | 7.40% | 14.80-15.08% | 2.00-2.04x |

Both gate statistics are over the 25% budget, so the reading does not depend on which one is
preferred. The mode ships, if it ships, as a user's explicit decision to spend headroom the
requirement does not grant, and the bench **reports without asserting** in Independent mode rather
than gating a figure NFR-PERF-010 does not state for it. §22 **R-19** carries the open risk.

*The first set, disqualified and kept on the record*, on this file's own convention and for the
same reason `six_stage_chain.rs`'s own doc comment keeps M9b's: two interleaved sets of 5
repetitions each, taken while this session's agent tooling was running, so **every one of the
twenty repetitions failed D-2.4 condition 2**. The estimator read 15.03% / 14.72% Linked against
30.08% / 30.86% Independent (2.00–2.10x); raw `p99.9` read 22.4–23.7% against 44.7–46.7%. Set
beside the certified run, the estimator moved by **under two points in either mode** while raw
`p99.9` moved by **six points Linked and fifteen Independent** — the argument for that estimator
made by the instrument rather than about it, and the second time this file has been able to make it
(the first is D-2.4's own M9b evidence). The ratio was right from the contaminated set; only its
precision was not.

*Consequence — load time and resident memory roughly double in the `Stereo` configuration, whether
or not the mode is used.* Per-channel state is preallocated unconditionally, which is what keeps the
live toggle RT-safe, and for Nam that means `new_state_prewarmed` (D-9.13) runs per channel: real
inference, not just allocation. Measured rather than assumed — `namir-worker`'s `resource_load`
bench, which prepares against `ChannelConfig::Stereo`, reports p99 12.9 ms for the standard model
and 198.6 ms for the 50 MB oversized fixture against NFR-PERF-050's 500 ms ceiling, so the
doubling is inside the budget with margin. Rejected as the alternative: prewarming channel 0 and
cloning it into the others, which would need `Clone` on `NamState` and its whole private scratch
tree for a cost the measurement says is affordable.

*Consequence — the toggle is a routing change and is not click-free, and this is documented rather
than smoothed.* Trim stops summing at −6 dB per term and passes each channel whole (up to +6 dB of
step); a channel whose gate envelope, model history and convolution tail have never run starts from
its reset or silence-prewarmed state. Each stage's per-channel *parameter* state is kept in step, so
thresholds and gains are already correct at the switch, but no crossfade covers the signal-state
discontinuity. Rejected: blending the two renderings over `BYPASS_CROSSFADE_TIME_CONSTANT_MS`, which
means running both topologies at once — a third of the cost budget again, for a transition a user
makes with the transport stopped.

*Consequence — one shared smoothing ramp per stage was a real defect, found by review of this very
change.* `GainRamp::process` advances one step per sample handed to it, so a ramp shared across
per-channel calls advances `n * channels` steps per block and hands channel 1 a *different* segment
of the glide than channel 0. `IrSlot::normalize` was exactly that shape and is now per physical
channel (`crates/namir-engine/src/stages/ir.rs`), regression-tested by feeding both channels the
same signal across an `ir.normalize_enabled` toggle and requiring identical output;
`NamSlot::normalize_gains` had it right from the start. Any future per-channel work in these stages
inherits the rule: **smoothed state is per channel, or it desynchronises the channels it smooths.**

*Consequence — telemetry stays single-channel.* Gate's gain-reduction readout and Trim's four meter
readouts report channel 0 only, in both modes. FR-IN-020/030 and FR-GATE-040 are written in the
singular and are satisfied by that; a second set of meters is a UI decision this mode does not
force.
...
| `cpal` (Namir fork) | 0.19.0 (`381cf1d`) | Apache-2.0 | 2026-09-08 | Standalone audio I/O; WASAPI exclusive mode | Low |
...
| R-10 | **New, from M8-planning, 2026-08-08.** D-13.4's forked `cpal` is a maintained fork: it must be rebased as upstream moves, and every rebase is work that produces no user-visible benefit and can silently regress the Windows audio path. It is also the first non-registry dependency in the tree, which weakens the reproducibility NFR-SEC-040 (Should) wants and needs a named `cargo-deny` `[sources]` allowance that the default policy would otherwise reject. **Status note (added M11, 2026-08-11, from building it) — the rebase burden is higher than D-13.4 assumed, and the upstreaming exit is in better shape than this row's own wording implies.** *Burden.* The fork as pinned (`rev 2edbacb`) is seven commits on upstream trunk `e0893c3`: **2867 insertions / 144 deletions** across seven files (`cpal-src/src/host/wasapi/`). Rebased on 0.18.1 trunk, not on an unreleased branch. **Status note (added 2026-09-09, issue #35):** Risk now extends to three Namir-maintained forks (`cpal`, `baseview`, `egui-baseview`). All three are strictly pinned by exact commit hash in manifests (`rev = "..."`) per D-13.4's mitigation rule, and allowed via narrow exact-URL entries in `deny.toml`. | Medium | Pin by commit hash in manifests; vendor if necessary. |
...
| R-19 | **New, from M15, 2026-09-09 — measured, not predicted, and the measurement is why the row exists.** D-9.14's independent-channel mode costs **~2x** the shipped chain and lands at **28.35-29.07%** of one core on the contamination-immune estimator and **31.39-31.82%** on raw `p99.9`, against NFR-PERF-010's **25%** budget — certified figures, §2's reference machine, quiet, 5 repetitions per mode with all ten quotable under D-2.4 (`six_stage_chain.rs` with `NAMIR_CHAIN_INDEPENDENT=1`). A user who turns it on is over the budget the product's only performance Must states — on the reference machine, so slower hardware is worse, and the standard WaveNet shape, so a larger model is worse again. The default is Linked and the bench asserts only Linked, so **no gate fires**: the way this reaches a user is a mode switch that works in the room where it was tried and drops blocks on a bigger model, a smaller buffer or a busier project. Namir has no per-mode budget, no headroom readout and no way to refuse the switch. | Medium | Three things, and the first is the honest one: **the mode is not advertised as inside NFR-PERF-010, in the FRS amendment, in D-9.14, in the parameter's own doc comment and in the bench's own output** — a user-selected departure from a budget, recorded as one, rather than a number quietly requoted. Second, the cost is *linear in captured channels* and Namir supports at most two, so this cannot get worse by degrees: 2x is the whole exposure, and it is measured rather than modelled. Third, the switch is engine-gated to `ChannelConfig::Stereo`, which keeps every `Mono`/`MonoToStereo` session — the whole standalone product — at the Linked cost whatever a preset says. **What is deliberately not mitigated**: nothing measures the *user's* actual headroom at runtime and nothing warns. Closing that means either a per-mode budget in the FRS (a requirements decision, not a code one) or a live CPU readout beside the control (FR-UI work no requirement asks for). Revisit if the mode is ever promoted from prototype to a shipped, documented feature; until then, the risk is that a prototype ships by inertia. |
...
| 0.53 | 2026-09-09 | **PR #162: independent per-channel processing, as an opt-in mode — new Decision D-9.14, new risk R-19, FR-CHAIN-050 amended by an appended Consequence in the FRS.** The case the mode exists for is one FR-CHAIN-050's Rationale does not reach: a `Stereo` input carrying two different performances (a DAW parent bus summing two panned double-tracked takes), where the mono core destroys a take rather than widening a source. `global.independent_channels` ("Linked" default) switches Gate, Trim, Nam and Ir to one instance per captured channel; every allocation happens in `prepare`/`NamSlot::new`/`IrSlot::new`, so the switch itself allocates nothing on the audio thread. **The RFC's deciding question is answered and the answer is "over budget"**: certified on §2's reference machine, quiet, 5 repetitions per mode with **all ten passing D-2.4's validity check**, so both of the bench's statistics speak: estimator **14.40% -> 28.35-29.07%** over two independent quiet runs, raw `p99.9` **16.76% -> 31.39-31.82%**, `p50` 7.40% -> 14.80-15.08% — ~2x, and over NFR-PERF-010's 25% **on both gate statistics**. An earlier contaminated set (two interleaved sets, every repetition discarded under D-2.4 condition 2) read 15.03% / 14.72% against 30.08% / 30.86% and is kept beside the certified one at D-9.14, because the comparison is itself evidence: the estimator moved under two points, raw `p99.9` moved six Linked and fifteen Independent. NFR-PERF-010 is **not** amended: `six_stage_chain.rs` still asserts the budget in Linked mode and now reports without asserting under `NAMIR_CHAIN_INDEPENDENT=1`. **Three defects found by review of the change itself, each fixed here.** (1) The predicate was `output_channels() > 1`, which engages on `MonoToStereo` — one captured channel duplicated — so a plugin-written preset opened in the standalone would have run two inferences over two identical signals: FR-CHAIN-050's doubled cost in exchange for precisely its "no benefit", on the product that cannot even reach the control. Now `input_channels()`, checked in the engine rather than trusted from the UI, which is also what makes the preset-portability answer "inert" rather than "hidden". (2) `IrSlot::normalize` was one `GainRamp` per *IR* channel, `process`ed once per physical channel per block — so it advanced twice per block of real time and handed channel 1 a different segment of the glide than channel 0, splitting L/R level for the whole of any `ir.normalize_enabled` toggle. Now per physical channel, with a regression test that feeds both channels the same signal across the toggle and requires identical output; the same first-block seating fixup now seats every channel rather than only the caller's. (3) The new mono-IR test compared against `namir_ir::direct_convolve` without defeating FR-IR-090, so it was red on the branch by exactly the 1.761x normalisation gain. Also: `global.independent_channels` reaches `PARAM_OVERRIDES` in FR-CFG-020's shell-parity vector pinned at "Linked", with the reason written down — the two shells run different channel configurations, so a non-default value would compare two different code paths; the golden preset is regenerated. FR-PARAM-050's "channel mode" category gains its first real citation, so the params test's named prototype exemption is deleted rather than kept. |
