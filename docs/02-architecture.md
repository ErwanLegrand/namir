...
| 0.43 | 2026-09-07 | **Issue #172: `SlimmableContainer` models are supported (D-9.12 consequence note added).** `namir-nam` recognises `architecture: "SlimmableContainer"`, validates submodel `max_value` ordering/terminal threshold and sample rates, and prepares the last submodel through `model::load`. `probe_metadata` extracts container/submodel metadata without allocating weights. `condition_dsp`, FiLM conditioning and `.namb` remain deferred. |
