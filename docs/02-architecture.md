...
worker thread (through `pool.shutdown`) before `destroy` returns, so no pool thread holding the
erased handle can call it after the pointer dies; and `request_callback` is one of `clap_host`'s
explicitly thread-safe operations.
