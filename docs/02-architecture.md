...
*Consequence — the mode costs 2.00–2.10x the shipped chain and exceeds NFR-PERF-010's budget.*
Measured with `crates/namir-engine/benches/six_stage_chain.rs` (`NAMIR_CHAIN_INDEPENDENT=1`), two
interleaved sets of 5 repetitions per mode on §2's reference machine, on D-2.4's
contamination-immune per-residue-minimum estimator:

| set | Linked | Independent | ratio |
|---|---|---|---|
| A | 15.03% of one core | 30.08% | 2.00x |
| B | 14.72% | 30.86% | 2.10x |

`p50` moved 8.0% -> 15.8-16.3% in step, and raw `p99.9` read 44.7-46.7% in Independent mode against
22.4-23.7% Linked. **Every repetition in both modes failed D-2.4's validity check** (this session's
own agent tooling was running, so the machine was not quiet), which is exactly why the estimator is
the figure quoted and why none of these is offered as a *certified* number — the comparison is
same-session and interleaved, which §2's own note says is the trustworthy form on this machine.
The conclusion does not depend on the residual uncertainty: **30% against a 25% budget is over
budget**, and the estimator cannot be inflated by load. So the mode ships, if it ships, as a user's
explicit decision to spend headroom the requirement does not grant, and the bench **reports without
asserting** in Independent mode rather than gating a figure NFR-PERF-010 does not state for it. §22
**R-19** carries the open risk.
