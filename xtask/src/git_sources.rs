...
/// Known exemptions: git sources that are allowed despite having no `?rev=`.
/// Currently empty; the rule is absolute in R-10's text.
const EXEMPT_SOURCES: &[&str] = &[];
...
pub fn scan_git_sources(root: &Path) -> Vec<String> {
            ...
            if let Some(source) = pkg_source {
                if let Some(name) = pkg_name {
                    if source.starts_with("git+")
                        && !source.contains("?rev=")
                        && !EXEMPT_SOURCES.contains(&source)
                    {
    ...
    if let Some(source) = pkg_source {
        if let Some(name) = pkg_name {
            ...
            {
                violations.push(format!(
                    "{name}: git source `{source}` has no `?rev=` pin — every git dependency must \
                     be pinned by commit hash (R-10), never by branch"
                ));
            }
        }
    }
